...
const MobileBottomSheet: React.FC<MobileBottomSheetProps> = ({ open, title, onClose, children, footer, header, headerAction, showHandle = true }) => {
  ...
  return createPortal(
    <div className="pointer-events-auto absolute inset-0 z-[1000] flex items-end justify-center bg-[var(--tm-mask)] animate-in fade-in [animation-duration:var(--tm-duration-standard)]">
      ...
      >
        ...
        <div className={`min-h-0 flex-1 overflow-y-auto px-[var(--tm-space-4)] no-scrollbar ${footer ? 'pb-[var(--tm-space-4)]' : 'pb-[calc(var(--tm-space-4)+env(safe-area-inset-bottom))]'}`}>{children}</div>
        {footer && (
