...
const AssistantClassSwitchButton: React.FC<AssistantClassSwitchButtonProps> = ({
    ...
    const variantClass = variant === 'quiet'
        ? 'px-1 text-[13px] font-medium text-[var(--tm-text-secondary)]'
        : 'px-2 text-[14px] font-semibold text-[var(--tm-text-primary)]';
    ...
    return (
        <button
            ...
            onClick={onClick}
            disabled={!switchable}
            className={`flex min-h-11 max-w-[190px] min-w-0 items-center gap-1 rounded-[var(--tm-radius-control)] disabled:cursor-default focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--tm-assistant-role-primary)] ${variantClass} ${className}`}
