...
import TeacherEvaluationReviewView from './TeacherEvaluationReviewView';

interface TeacherEvaluationReviewHistoryViewProps {
    onBack: () => void;
    classes: ClassInfo[];
...
const groupReviewsByYear = (reports: TeacherEvaluationReviewReport[]) => [...reports]
    ...
    .reduce<ReviewYearGroup[]>((groups, report) => {
        const year = report.reviewMonth.split('-')[0];
        const existingGroup = groups.find((group) => group.year === year);
        if (existingGroup) existingGroup.reports.push(report);
        else groups.push({ year, reports: [report] });
        return groups;
    }, []);

const TeacherEvaluationReviewHistoryView: React.FC<TeacherEvaluationReviewHistoryViewProps> = ({
    onBack,
    classes,
    initialClassId,
}) => {
    ...
    const reports = TEACHER_EVALUATION_REVIEW_HISTORY_BY_CLASS[activeClass?.id] ?? [];
    const yearGroups = useMemo(() => groupReviewsByYear(reports), [reports]);
    ...
    return (
        <div className="relative min-h-full overflow-hidden bg-[linear-gradient(180deg,#F7FCFF_0%,#F1F8FC_42%,#FFFFFF_100%)] font-sans text-slate-950">
            ...
            <main className="relative z-10 px-5 pb-10 pt-2">
                ...
                <section className="space-y-5" aria-label="往期评价复盘列表">
                    {yearGroups.map((group) => (
                        <section key={group.year} aria-labelledby={`review-year-${group.year}`}>
                            <div className="relative flex h-6 items-center pl-5">
                                ...
                                <h2 id={`review-year-${group.year}`} className="text-[13px] font-semibold text-slate-500">{group.year}年</h2>
                            </div>
                    ...
                    {reports.length === 0 && (
                        <p className="py-16 text-center text-[13px] text-slate-400">暂无往期复盘</p>
                    )}
                </section>
            </main>
...
export default TeacherEvaluationReviewHistoryView;
