...
export const sectionSurface = 'rounded-[var(--tm-radius-card)] border border-[var(--tm-border-subtle)] bg-[var(--tm-bg-surface)] [box-shadow:var(--tm-shadow-card)]';
export const primaryButton = 'inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[var(--tm-radius-control)] bg-[var(--tm-brand-primary)] px-5 text-[15px] font-bold text-white [box-shadow:var(--tm-shadow-card)] transition active:scale-[0.98] active:bg-[var(--tm-brand-primary-strong)] disabled:cursor-not-allowed disabled:opacity-45 disabled:shadow-none';
...
export const BottomSheet: React.FC<{ open: boolean; label: string; onDismiss: () => void; children: React.ReactNode }> = ({ open, label, onDismiss, children }) => {
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-end bg-[var(--tm-mask)] backdrop-blur-[2px]" onClick={onDismiss}>
      <section className="max-h-[88%] w-full overflow-y-auto rounded-t-[var(--tm-radius-sheet)] bg-[var(--tm-bg-surface)] px-5 pb-[calc(20px+env(safe-area-inset-bottom))] pt-3 [box-shadow:var(--tm-shadow-sheet)]" onClick={event => event.stopPropagation()} role="dialog" aria-modal="true" aria-label={label}>
...
export const Toast: React.FC<{ message: string }> = ({ message }) => {
  ...
  return <div className="pointer-events-none absolute inset-x-5 bottom-24 z-[70] rounded-[15px] bg-[var(--tm-text-primary)] px-4 py-3 text-center text-[13px] font-semibold text-[var(--tm-text-inverse)] [box-shadow:var(--tm-shadow-card-raised)]">{message}</div>;
};
