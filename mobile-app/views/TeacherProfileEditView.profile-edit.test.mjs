...
requireText(editSource, 'rounded-t-[var(--tm-radius-sheet)]', '底部浮层弹窗应贴底并只保留顶部大圆角，圆角引用组件令牌。');
...
requireText(editSource, 'text-right text-sm font-bold text-[var(--tm-text-primary)]', '除任教班级外的配置值应靠右展示。');
requireText(editSource, 'renderConfigValue', '未选择态配置值应统一弱化展示。');
requireText(editSource, 'font-normal text-[var(--tm-text-tertiary)]', '未选择态“暂未选择”应弱化且不加粗，并使用满足对比度的弱文字令牌。');
...
for (const legacyColor of ['slate-', 'blue-', 'indigo-', 'violet-', 'purple-', 'cyan-', 'rose-']) {
  if (editSource.includes(legacyColor)) {
    throw new Error(`教师信息编辑页仍残留旧视觉颜色：${legacyColor}，应统一改用教师手机端 --tm-* 令牌。`);
  }
}
...
for (const requiredToken of [
  ...
  'border-[var(--tm-border-control)]',
  'text-[var(--tm-text-tertiary)]',
  'text-[var(--tm-text-disabled)]',
  'active:text-[var(--tm-status-negative)]',
]) {
  requireText(editSource, requiredToken, `教师信息编辑页应接入教师手机端语义令牌：${requiredToken}`);
}
