...
requireText(dataSource, "overview: { records: 24, covered: 19, total: 60, evaluators: 4 }", '中等数据报告 mock 应完整。');
requireText(dataSource, "dataRange: '6月23日-7月6日'", '中等数据报告应使用近14天记录。');
requireText(dataSource, "dataWindowWeeks: 4", '应包含前四周窗口的谨慎报告。');
...
for (const forbidden of ["status: 'accumulating'", 'summary:', 'accumulationTips']) {
  if (dataSource.includes(forbidden)) throw new Error(`报告数据不应包含未生成状态或摘要：${forbidden}`);
}

requireText(prdSource, '前1周、前2周或前4周', 'PRD 应说明完整自然周的自适应数据窗口。');
