...
assert.ok(classListSource.includes('className="inline-flex h-[var(--tm-class-list-leaderboard-touch-height)] w-[var(--tm-class-list-leaderboard-touch-width)] shrink-0 items-center justify-self-end justify-center rounded-none'), '排行榜长条图片入口应固定在筛选区第一行右侧，并保留44像素触控高度。');
assert.ok(classListSource.includes('aria-label="查看班级排行榜"'), '排行榜入口必须保留完整读屏名称。');
assert.ok(classListSource.includes('src={ASSETS.CLASS.LEADERBOARD_ENTRY}'), '排行榜入口应使用统一图片资源。');
assert.ok(classListSource.includes('className="h-[var(--tm-class-list-leaderboard-image-height)] w-[var(--tm-class-list-leaderboard-image-width)] select-none object-contain"'), '排行榜图片应使用112×24像素可视区域并完整显示。');
assert.ok(!classListSource.includes('CLASS_LEADERBOARD'), '班级页不应继续依赖之前的排行榜悬浮图片素材。');
assert.ok(tokenSource.includes("'--tm-class-list-leaderboard-touch-width': '120px'"), '排行榜触控区宽度应由班级页令牌统一管理。');
assert.ok(tokenSource.includes("'--tm-class-list-leaderboard-image-width': '112px'"), '排行榜占位图宽度应由班级页令牌统一管理。');
assert.ok(tokenSource.includes("'--tm-class-list-leaderboard-image-height': '24px'"), '排行榜占位图高度应使用24像素紧凑比例。');
...
assert.ok(toolbarSource.includes('inline-flex h-[var(--tm-size-touch)] w-[96px] shrink-0 items-center gap-['), '年级筛选应保持开放式内容并保留44像素触控高度。');
...
assert.ok(toolbarSource.includes('className={`flex h-[var(--tm-size-touch)] shrink-0 items-center gap-[var(--tm-space-1)] whitespace-nowrap rounded-[var(--tm-radius-control)]'), '任教班级应保持开放式轻量样式并保留44像素触控高度。');
...
assert.ok(toolbarSource.includes('共{visibleClasses.length}个班级'), '筛选区应展示“共x个班级”的当前可见班级数量。');
