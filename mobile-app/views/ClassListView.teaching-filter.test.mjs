...
if (classListSource.includes('rounded-2xl text-sm font-semibold shadow-md')) {
  throw new Error('班级排行榜入口不应继续使用16px圆角和46px高度。');
}
if (classListSource.includes('w-11 shrink-0 items-center justify-self-end justify-center')) {
  throw new Error('班级排行榜入口不应继续使用方形图片按钮。');
}
