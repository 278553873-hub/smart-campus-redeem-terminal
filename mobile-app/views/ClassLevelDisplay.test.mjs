...
assert.match(classDetailSource, /label: '本学期'/, '当前学期选项应使用简洁的数据范围文案。');
assert.match(classDetailSource, /label: '历史累计'/, '累计选项应使用简洁的数据范围文案。');
assert.match(classDetailSource, /<CompactSegmentedControl[\s\S]*items=\{LEVEL_DISPLAY_OPTIONS\}[\s\S]*onChange=\{onUpdateStudentLevelDisplayMode\}[\s\S]*ariaLabel="等级统计范围"[\s\S]*semantics="group"[\s\S]*variant="settings"/, '等级统计范围应复用设计系统设置项分段控件。');
