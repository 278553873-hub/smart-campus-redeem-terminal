  ...
  <div className="space-y-2.5 pb-24 pt-2 animate-in fade-in duration-200">
    {items.map(item => {
      ...
      return (
        <button
          ...
          type="button"
          onClick={() => onOpen(item)}
          className="relative min-h-[88px] w-full overflow-hidden rounded-[var(--tm-radius-inner)] bg-white px-4 py-4 text-left shadow-[var(--tm-shadow-card)] transition-[transform,background-color,box-shadow] duration-150 active:scale-[0.98] active:bg-[var(--tm-bg-surface-soft)]"
        >
          <span className={`pointer-events-none absolute inset-y-3 left-0 w-[3px] rounded-r-full ${meta.accentClass}`} aria-hidden="true" />
          <div className="line-clamp-2 text-pretty text-[15px] font-semibold leading-[21px] text-[var(--tm-text-primary)]">{item.title}</div>
    ...
    {items.length === 0 && (
      <div className="py-14 text-center">
        ...
        <div className="mt-4 text-[15px] font-semibold text-[var(--tm-text-secondary)]">暂无采集记录</div>
      </div>
    )}
  </div>
);
