...
requireText(meSource, 'ClassSourceIcon', '个人、协作和学校来源应使用统一来源图标。');
requireText(meSource, 'const getClassSourceTypeLabel', '来源类型应保留在读屏语义中，而不是重复展示为文字标签。');
requireText(meSource, 'getTeacherSchoolTypeLabel(space)', '学校来源应展示学校类型标签。');
requireText(meSource, 'schoolTypeLabel &&', '学校类型标签应仅在配置了学校类型时展示。');
...
requireText(sourceTriggerSource, 'export const ClassSourceIcon', '来源图标应作为公共组件导出，供弹窗复用。');
...
requireText(sheetSource, '<MobileBottomSheet open title="切换班级来源" onClose={onClose} contentTone="plain">', '班级来源抽屉内容区应使用统一浅灰底。');
requireText(sheetSource, 'space-y-3 py-[var(--tm-space-3)]', '来源列表应在浅灰内容区内保留上下内边距。');
requireText(sheetSource, 'text-[length:var(--tm-font-size-body)] font-medium leading-5', '所有来源名称应统一使用14px字号、20px行高和500字重。');
...
requireText(appSource, "{ id: 'school-qizhong', title: '成都七中初中附属小学', type: 'school', schoolType: 'primary', role: 'homeroomTeacher',", '成都七中初中附属小学应标记为小学。');
requireText(appSource, "{ id: 'school-star', title: '星河实验学校', type: 'school', schoolType: 'twelveYear', role: 'leader',", '星河实验学校应标记为十二年一贯制。');
requireText(appSource, "{ id: 'school-qinghe', title: '青禾实验小学', type: 'school', schoolType: 'primary', role: 'homeroomTeacher',", '其他学校来源示例应保留小学类型和班主任身份。');
