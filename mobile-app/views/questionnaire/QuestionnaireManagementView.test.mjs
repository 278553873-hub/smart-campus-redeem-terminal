...
requireText(viewSource, "<Plus className=\"h-4 w-4\" />{isStudentCollection ? '添加字段' : '添加题目'}", '编辑区域右上角必须保留唯一的添加入口。');
if (viewSource.includes('label="复制题目"')) {
  throw new Error('题目编辑器不应保留复制题目功能。');
}
requireText(viewSource, 'setPendingQuestionFocusId(question.id)', '选择题型后必须直接聚焦新题目的输入框。');
requireText(viewSource, 'border border-[var(--tm-border-control)] bg-[var(--tm-bg-surface)]', '题目和选项输入框必须使用教师端控件边框与表面令牌。');
requireText(viewSource, '>{optionIndex + 1}</span>', '选项编辑器应使用中性序号，不应模拟家长作答控件。');
if (viewSource.includes('<GripVertical') || viewSource.includes("question.type === 'single' ? 'rounded-full' : 'rounded-[4px]'")) {
  throw new Error('题目配置页不应保留无效拖拽图标或可勾选答案的视觉暗示。');
}
...
requireText(viewSource, 'const MIN_RATING_LEVELS = 2;', '量表最少必须支持2级。');
requireText(viewSource, 'const MAX_RATING_LEVELS = 10;', '量表最多必须支持10级。');
requireText(viewSource, 'updateRatingLevelCount', '量表题必须支持配置级数。');
...
requireText(viewSource, 'sticky top-0 z-[45] flex h-11 shrink-0 items-center justify-between border-b border-[var(--tm-border-subtle)] bg-[var(--tm-bg-page-glass)] px-4 backdrop-blur-md', '问卷顶部必须使用教师端44像素头部、背景和分割线令牌。');
...
requireText(viewSource, "showMoreFieldTypes ? ['text', 'number', 'multiple'] : ['short_text', 'single', 'date']", '字段选择必须优先展示常用字段，并渐进披露其他字段。');
