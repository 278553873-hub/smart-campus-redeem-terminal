...
assert.match(loginSource, /h-\[84px\] w-\[84px\].*rounded-full/, '登录视觉资源应以小尺寸正圆产品 Icon 展示。');
assert.match(loginSource, /className="h-full w-full rounded-full object-cover outline outline-1 -outline-offset-1 outline-black\/10"/, '登录 Icon 图片应使用正圆裁切与低透明纯黑内描边。');
assert.doesNotMatch(loginSource, /h-\[84px\] w-\[84px\][^\n]*border/, '登录 Icon 外层不应保留方形边框。');
...
assert.match(loginSource, /wechatLoginButtonClass = '[^']*focus-visible:ring-\[var\(--tm-focus-ring\)\]/, '一键登录按钮应使用设计 Token 聚焦环。');
assert.match(loginSource, /recentLoginButtonClass = '[^']*focus-visible:ring-\[var\(--tm-focus-ring\)\]/, '最近登录按钮应使用设计 Token 聚焦环。');
...
assert.doesNotMatch(loginSource, /transition-all/, '登录页交互不得使用 transition-all。');
