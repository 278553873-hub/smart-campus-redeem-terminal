# 班级列表图标

这里列出“班级列表”页面及其弹窗、抽屉和复用子组件实际使用的图标。公共图标只在公共目录保存一份。

| 资源路径 | 页面用途 | 存放位置 | 类型 |
| --- | --- | --- | --- |
| `../../00-公共图标/bar-chart-3.svg` | 进入班级报告 | 公共图标 | Lucide SVG |
| `../../00-公共图标/building-2.svg` | 学校来源 | 公共图标 | Lucide SVG |
| `../../00-公共图标/check.svg` | 班级号复制成功 | 公共图标 | Lucide SVG |
| `../../00-公共图标/chevron-down.svg` | 展开或下拉 | 公共图标 | Lucide SVG |
| `../../00-公共图标/chevron-left.svg` | 返回 | 公共图标 | Lucide SVG |
| `../../00-公共图标/chevron-right.svg` | 进入下一级 | 公共图标 | Lucide SVG |
| `../../00-公共图标/copy.svg` | 复制班级号 | 公共图标 | Lucide SVG |
| `../../00-公共图标/download.svg` | 下载 | 公共图标 | Lucide SVG |
| `../../00-公共图标/file-text.svg` | 作业录入 | 公共图标 | Lucide SVG |
| `../../00-公共图标/folder-archive.svg` | 批量留档 | 公共图标 | Lucide SVG |
| `../../00-公共图标/gift.svg` | 兑换奖励 | 公共图标 | Lucide SVG |
| `link.svg` | 链接邀请 | 本页面 | Lucide SVG |
| `log-in.svg` | 加入班级 | 本页面 | Lucide SVG |
| `../../00-公共图标/message-circle.svg` | 邀请家长加入 | 公共图标 | Lucide SVG |
| `../../00-公共图标/more-horizontal.svg` | 打开班级更多操作 | 公共图标 | Lucide SVG |
| `../../00-公共图标/plus.svg` | 新增 | 公共图标 | Lucide SVG |
| `../../00-公共图标/qr-code.svg` | 二维码 | 公共图标 | Lucide SVG |
| `../../00-公共图标/scan-face.svg` | 更新人脸数据 | 公共图标 | Lucide SVG |
| `../../00-公共图标/send.svg` | 发送或提交 | 公共图标 | Lucide SVG |
| `../../00-公共图标/shield.svg` | 设置兑换密码 | 公共图标 | Lucide SVG |
| `../../00-公共图标/sliders-horizontal.svg` | 班级显示管理 | 公共图标 | Lucide SVG |
| `../../00-公共图标/trophy.svg` | 排行榜 | 公共图标 | Lucide SVG |
| `../../00-公共图标/user-plus.svg` | 邀请老师加入 | 公共图标 | Lucide SVG |
| `../../00-公共图标/user-round.svg` | 个人来源或学生 | 公共图标 | Lucide SVG |
| `../../00-公共图标/users-round.svg` | 协作来源或多人 | 公共图标 | Lucide SVG |
| `../../00-公共图标/users.svg` | 学生列表、批量修改、离校学生管理 | 公共图标 | Lucide SVG |
| `../../00-公共图标/x.svg` | 关闭 | 公共图标 | Lucide SVG |
