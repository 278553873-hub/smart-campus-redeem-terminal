...
requireSource("if (pageKey === 'classActionTeacherPreview') return '07C';", '普通老师共用更多操作应编号为 07C。');
requireSource("if (pageKey === 'classActionManagerPreview') return '07D';", '班主任/副班主任共用更多操作应编号为 07D。');
requireSource("{ title: '班级操作', pages: ['classActionTeacherPreview', 'classActionManagerPreview'], parallel: true }", '页面导航地图应合并展示两个共用班级操作弹层。');
...
if (!classListRender.includes("const isSchoolList = isSchoolClassListPage(page);")) {
  failures.push('班级列表页必须通过共用页面判断区分 07A 个人版和 07B 学校版。');
}
...
requireSource("const [showSchoolNameSheet, setShowSchoolNameSheet] = useState(false);", '15A 应支持编辑学校名称。');
requireSource("aria-label=\"修改学校\"", '15A 学校行应可编辑并打开修改学校弹窗。');
requireSource("const displayedSchoolName = isPersonalBasicInfo ? (teacherSchoolName.trim() || '星河实验小学') : getTeacherSpaceDisplayName(currentSpace);", '15A 应展示 02 页填写学校，15B 应展示当前正式学校来源。');
...
requireSource("modules: ['教师信息', '头像', '姓名', '扫一扫', '设置', '班级来源入口'", '14B 应展示教师信息、扫一扫、设置、班级来源入口。');
...
requireSource("const [userHasMultipleClassSources, setUserHasMultipleClassSources] = useState(true);", '应有登录用户是否拥有多个班级来源的状态。');
requireSource("if (!showSpaceSelectSheet || !userHasMultipleClassSources) return null;", '单来源用户不应渲染班级来源切换弹层。');
requireSource("userHasMultipleClassSources && (", '记录页、班级页和我的页的切换入口应仅在多来源用户下展示。');
requireSource("aria-label=\"切换班级来源\"", '班级来源弹层 aria-label 应为切换班级来源。');
...
requireSource("<h3 className=\"text-base font-black\">切换班级来源</h3>", '班级来源弹层标题应为切换班级来源。');
requireSource("const teacherSpaces: TeacherSpaceProfile[] = [", '空间切换应来自统一空间列表。');
requireSource("{ id: 'personal', title: '我创建的', type: 'personal' }", '班级来源列表应包含我创建的。');
requireSource("{ id: 'collabLi', title: '李明老师', type: 'collaboration' }", '班级来源列表应包含协作老师来源。');
requireSource("const getTeacherSpaceDisplayName = (space: TeacherSpaceProfile) => {", '应有面向老师的班级来源展示名称。');
requireSource("if (space.type === 'collaboration') return `${space.title}的班级`;", '协作来源应展示为某某老师的班级。');
requireSource("return '我创建的班级';", '个人来源应展示为我创建的班级。');
...
requireSource("selectTeacherSpace(spaceId, page === 'mineSchool' ? 'mineSchool' : undefined)", '14B 代表用户拥有多个班级来源，切换来源后也应停留在 14B。');
...
requirePrd('  - 15A 个人版展示 02 页填写的学校名称，可编辑；该字段仅作为老师资料和线索字段，不代表正式学校空间', 'PRD 应明确 15A 学校字段可编辑且不是正式学校空间。');
requirePrd('  - 15B 学校版展示正式学校名称，只读不可编辑', 'PRD 应明确 15B 学校名称只读。');
requirePrd('  - 右上角 → 扫一扫 icon、设置 icon', 'PRD 应明确右上角有扫一扫和设置 icon。');
requirePrd('  - 切换班级来源弹窗 → 我创建的 / 协作老师 / 多个学校，列表只展示来源名称、个人/协作/学校标签和当前标签，不展示备注文案', 'PRD 切换班级来源弹窗应支持个人、协作、学校来源且禁止备注文案。');
requirePrd('  - 只有登录用户拥有多个班级来源时，记录页、班级页和我的页才展示切换入口；单来源用户只在我的页基本信息处展示当前来源', 'PRD 应明确单来源不在记录页和班级页展示切换入口。');
...
if (!mineRender.includes("const hasMultipleVersions = page === 'mineSchool';")) {
  failures.push('14B 应代表用户拥有多个班级来源，不能只按当前切换来源判断页面身份。');
}
...
if (!mineRender.includes("aria-label=\"进入基本信息设置\"") || !mineRender.includes("aria-label=\"切换班级来源\"") || !mineRender.includes("!userHasMultipleClassSources && (")) {
  failures.push('我的页必须具备头像基本信息入口，多来源展示切换入口，单来源展示当前来源。');
}
if (mineRender.includes("{getTeacherSpaceTag(currentSpace)}")) {
  failures.push('14A/14B 我的页选中态不应显示个人/协作/学校标签。');
}
