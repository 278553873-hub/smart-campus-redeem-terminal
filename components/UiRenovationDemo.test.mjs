...
assert.match(demo, /section: 'showcase'/);
...
assert.match(demo, /renovationSections\.map/);
assert.match(demo, /activeDemos\.map/);
...
assert.match(componentInventory, /结论：部分统一，复用边界未收敛/);
...
assert.equal(componentAudit.sourceFileCount, 171, '组件盘点应记录扫描源码文件数。');
assert.equal(componentAudit.componentDefinitionCount, 596, '组件盘点应记录组件定义数。');
assert.equal(componentAudit.importRelationCount, 1648, '组件盘点应记录组件导入关系数。');
assert.equal(componentAudit.reuseHotspots.length, 7, '组件复用热点不可缺项。');
assert.equal(componentAudit.duplicateCandidates.length, 5, '组件重复实现候选分类不可缺项。');
assert.deepEqual(componentAudit.studentPickerContract.scenarios, ['添加小组', '调整成员', '颁发奖章', '批量操作']);
...
assert.match(termReportRedesign, /five-education-detail', label: '五育成长表现', defaultEnabled: true/);
assert.match(termReportRedesign, /section-five-education-detail/);
assert.match(termReportRedesign, /fiveEducationDetailContent=\{<FiveEducationDetailCard \/>\}/);
assert.match(termReportRedesign, /<ConfiguredSectionCard title="五育成长表现" icon=\{Layers\}>/);
for (const dimension of ['成正', '成健', '成智', '成雅', '成技']) {
  assert.match(termReportRedesign, new RegExp(dimension), `五育成长表现应包含${dimension}指标。`);
}
assert.doesNotMatch(termReportRedesign, /badge="记录解读"/);
assert.doesNotMatch(termReportRedesign, /Five Education Review/);
assert.doesNotMatch(termReportRedesign, /依据本学期行为记录，逐项解读五个一级指标的成长表现/);
assert.match(termReportRedesign, /成长证据正在积累/);
assert.match(termReportRedesign, /从关注点开始持续改进/);
assert.match(termReportRedesign, /tone: BehaviorRecordTone/);
assert.doesNotMatch(termReportRedesign, /我的五育/);
assert.match(termReportRedesign, /medals', label: '成长奖章', defaultEnabled: true/);
assert.match(termReportRedesign, /<span>\{isEditing \? '完成' : '编辑'\}<\/span>/);
assert.match(termReportRedesign, /添加奖章/);
assert.match(termReportRedesign, /成正少年/);
assert.match(termReportRedesign, /成智少年/);
assert.match(termReportRedesign, /成健少年/);
assert.match(termReportRedesign, /成雅少年/);
assert.match(termReportRedesign, /成技少年/);
assert.match(termReportRedesign, /删除\$\{medal.name\}/);
assert.match(termReportRedesign, /role="dialog"/);
assert.match(termReportRedesign, /MobileBottomSheet/);
assert.match(termReportRedesign, /CompactSegmentedControl/);
assert.match(termReportRedesign, /平台奖章/);
assert.match(termReportRedesign, /学校奖章/);
assert.match(termReportRedesign, /班级奖章/);
assert.match(termReportRedesign, /Math\.max\(1, medal\.quantity - 1\)/);
assert.match(termReportRedesign, /medal.quantity > 1/);
assert.match(termReportRedesign, /grid-cols-3 gap-x-3 gap-y-4/);
assert.match(termReportRedesign, /relative w-full min-w-0 text-center/);
assert.match(termReportRedesign, /absolute right-1 top-1 rounded-full/);
assert.match(termReportView, /showEnglish=\{false\}/);
assert.match(termReportView, /hideFooterLabel=\{!showEnglish\}/);
assert.match(termReportView, /configuredOverviewHeader/);
assert.match(termReportView, /configured-overview-header mb-5 flex justify-center py-1/);
assert.match(termReportView, /text-2xl font-black leading-tight text-slate-900">成长概览/);
assert.match(termReportView, /showSubjectClickHint=\{false\}/);
assert.match(termReportRedesign, /hideEndMarker/);
assert.match(termReportView, /hideEndMarker = false/);
assert.match(termReportView, /!hideEndMarker/);
assert.match(termReportRedesign, /report-medals-card/);
assert.match(termReportRedesign, /compactConfiguredMobileSections/);
...
assert.match(termReportView, /dimensionLabels=\{\['成正', '成健', '成智', '成雅', '成技'\]\}/);
assert.match(termReportView, /report-page-container/);
assert.match(termReportView, /configured-report-preview/);
assert.match(termReportView, /const growthSectionIds: TermReportSectionId\[\]/);
assert.match(termReportView, /flushGrowthSectionGroup\(\)/);
assert.match(termReportView, /showConfiguredOverviewIntro=\{growthPageIndex === 0\}/);
assert.doesNotMatch(termReportView, /六育发展亮点|六维度发展数据|从六维度发展雷达图来看/);
...
assert.match(termReportView, /additionalMedalMobilePages\?: React\.ReactNode\[\]/);
assert.match(termReportView, /additionalMedalA4Pages\?: React\.ReactNode\[\]/);
assert.match(termReportView, /'medals'/);
