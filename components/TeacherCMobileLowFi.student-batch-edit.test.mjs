...
requireText("const canSaveStudentBatch = studentBatchEditRows.length > 0 && studentBatchEditRows.every((row) => row.name.trim() && row.no.trim() && row.gender);", '姓名、学号和性别有效时才可保存，学号必填。');
requireText('placeholder="学号必填"', '批量修改页学号输入应明确为学号必填。');
requireText('aria-required="true"', '批量修改页学号输入应标记为必填。');
...
requireText('姓名、学号和性别都有值时，底部保存修改按钮可点击；学号必填。', 'PRD 应说明批量修改页学号必填。');
