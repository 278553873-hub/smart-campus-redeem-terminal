...
requireText("const canSubmitStudents = studentInputRows.every((row) => row.name.trim() && row.no.trim() && row.gender);", '姓名、学号和性别完成后才可提交，学号必填。');
...
requireText('placeholder="学号必填"', '学号输入应明确为学号必填。');
requireText('aria-required="true"', '学号输入应标记为必填。');
...
requireText('学号必填，填写后可支持学号进行评价，例如“1号和3号同学打架”', '12 添加学生应说明学号必填且用于按编号评价。');
...
requireText('每一行的姓名、学号和性别填写完成后，底部完成按钮才可点击；学号必填。', 'PRD 应说明学号必填且阻断提交。');
