...
const ParentApp: React.FC<ParentAppProps> = ({ showPhoneShell = true }) => {
  ...
  const ParentBottomNav = () => {
    ...
    const goTab = (item: (typeof tabItems)[number], nextIndex: number) => {
      ...
      setParentNavJellyToggle(prev => (prev === 'a' ? 'b' : 'a'));
      setScreen(item.key);
    };

    return (
      <nav
        className={`ai-tabbar-container ${parentNavJellyToggle !== 'none' ? `tabbar-jelly-active-${parentNavJellyToggle}` : ''}`}
        aria-label="家长端底部导航"
      >
        <TeacherFluidGlassNav activeIndex={activeIndex} itemCount={tabItems.length} jellyToggle={parentNavJellyToggle} />
        {tabItems.map(item => {
          const Icon = item.icon;
