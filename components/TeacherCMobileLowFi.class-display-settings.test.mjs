...
requireText("const createdClassCards = visiblePersonalClassCards.filter((item) => item.isCreator);", '07A 创建的班级分组应基于已显示班级。');
requireText("const joinedClassCards = visiblePersonalClassCards.filter((item) => !item.isCreator);", '07A 加入的班级分组应基于已显示班级。');
...
const personalOnlyStart = classListRender.indexOf('{!isSchoolList && (');
...
if (!personalTopBlock.includes('className="flex items-center justify-end"') || !personalTopBlock.includes('className="flex h-11 w-11 items-center justify-center rounded-full')) {
