...
requireText("type ParentPageKey = 'wechatCard' | 'login' | 'bindSelfMatched' | 'bindSelfUnmatched' | 'bindInviteMatched' | 'bindInviteUnmatched' | 'bindInviteBoundUnmatched' | 'bindingLimitNotice' | 'bindingNotice' | 'parentIdentity' | 'landing';", '家长端被邀请手机号未匹配应按班级已绑定数量进入 03B/03C/04B。');
...
requireText("text: '未绑定任何学生&手机号匹配成功'", '01 后应直接展示未绑定且手机号匹配成功分支。');
requireText("text: '未绑定任何学生&手机号未匹配'", '01 后应直接展示未绑定且手机号未匹配分支。');
requireText("{ text: '已绑定任何学生', pages: [{ page: 'landing' }] }", '01 后应直接展示已绑定任何学生分支。');
...
requireText("{ text: '手机号匹配成功', pages: [{ page: 'bindInviteMatched' }, { page: 'landing' }] }", '被邀请手机号匹配成功分支应从 01 微信登录进入 03A。');
...
requireText("text: '检索到有新的绑定信息'", '自主新增绑定应展示检索到新绑定信息分支。');
requireText("text: '未检索到新的绑定信息'", '自主新增绑定应展示未检索到新绑定信息分支。');
requireText("{ text: '检索到有新的绑定信息', pages: [{ page: 'bindSelfMatched' }, { page: 'landing' }] }", '检索到新绑定信息后应进入 03A 并回到 05。');
requireText("{ text: '未检索到新的绑定信息', pages: [{ page: 'bindSelfUnmatched' }] }", '未检索到新绑定信息后应进入 02B 提示页。');
...
requireText("const allowPhone = (phone: string) =>", '家长端选择不同手机号应能进入不同匹配结果。');
requireText("jumpToParentPage(target);", '家长端选择手机号后应关闭弹窗并进入当前入口对应的匹配结果页。');
requireText("parentLoginTarget === 'bindInviteMatched' ? (hasReachedClassBindingLimit ? 'bindingLimitNotice' : 'bindInviteUnmatched') : 'bindSelfUnmatched'", '被邀请手机号未匹配且该班已绑定 2 个学生应进入 04B。');
requireText("const hasReachedClassBindingLimit = phone === '19900008610';", '被邀请授权弹窗应能演示该班已绑定 2 个学生的 04B 分支。');
...
requireText("if (parentInviteStudentComplete) {\n              setParentBindingNoticeBackPage(variant === 'bound' ? 'bindInviteBoundUnmatched' : 'bindInviteUnmatched');\n              jumpToParentPage('bindingNotice');", '03B/03C 绑定学生后应进入 04A 绑定提醒。');
...
requireText('const startParentBindingLookup = (hasResult = true) =>', '新增绑定应先拉取可绑定学生。');
requireText("setParentBindingReturnPage('landing');", '05 落地页新增绑定拉取后的结果页应返回 05。');
requireText("showClassActionToast('正在查找可绑定学生');", '05 落地页新增绑定应使用 toast 展示检索进度。');
requireText("jumpToParentPage(hasResult ? 'bindSelfMatched' : 'bindSelfUnmatched');", '新增绑定拉取后应根据结果进入确认绑定或 02B 提示页。');
