...
type PageKey =
  ...
  | 'classListPersonal'
  | 'classListSchool'
  | 'classActionTeacherPreview'
  | 'classActionManagerPreview'
  | 'classDetail'
  | 'classDetailMember'
...
type TeacherSuggestionFeedback = {
  id: string;
  teacher: string;
  phone: string;
  version: '个人版' | '学校版';
...
type FlowLane = { title: string; pages: PageKey[]; parallel?: boolean; branchGroups?: Array<{ branches: FlowBranch[] }>; tailPages?: PageKey[] };
...
const flowLanes: FlowLane[] = [
    ...
    branchGroups: [
      {
        branches: [
          ...
          { text: '副班主任', pages: ['classDetailDeputy'] },
          { text: '普通老师', pages: ['classDetailMember'] },
        ],
      },
      {
        branches: [
          { text: '班主任', pages: ['teacherList'] },
          { text: '副班主任', pages: ['teacherListDeputy'] },
          { text: '普通老师', pages: ['teacherListMember'] },
        ],
      },
      {
        branches: [
          { text: '班主任&副班主任', pages: ['parentBindingList'] },
          { text: '普通老师', pages: ['parentBindingListMember'] },
        ],
      },
    ],
    tailPages: ['studentList', 'studentBatchEdit'],
  },
  {
    title: '班级(学校版)',
  ...
  { title: '班级操作', pages: ['classActionTeacherPreview', 'classActionManagerPreview'], parallel: true },
  { title: '我的(个人版)', pages: ['minePersonal', 'teacherBasicInfoPersonal', 'suggestionFeedback'] },
  { title: '我的(学校版)', pages: ['mineSchool', 'teacherBasicInfoSchool', 'mineSettings', 'subjectManagement', 'departmentManagement', 'coinIssuanceManagement', 'suggestionFeedback'] },
];

const pageNumberLabel = (pageKey: PageKey) => {
  if (pageKey === 'profile') return '02A';
  if (pageKey === 'profileInvite') return '02B';
  if (pageKey === 'classListPersonal') return '07A';
  if (pageKey === 'classListSchool') return '07B';
  if (pageKey === 'classActionTeacherPreview') return '07C';
  if (pageKey === 'classActionManagerPreview') return '07D';
  if (pageKey === 'classDetail') return '08A';
  if (pageKey === 'classDetailMember') return '08B';
  if (pageKey === 'classDetailSchoolHead') return '08C';
  if (pageKey === 'classDetailDeputy') return '08D';
  if (pageKey === 'teacherList') return '09A';
  if (pageKey === 'teacherListMember') return '09B';
  if (pageKey === 'teacherListDeputy') return '09C';
  if (pageKey === 'parentBindingList') return '10A';
  if (pageKey === 'parentBindingListMember') return '10B';
  ...
  if (pageKey === 'suggestionFeedback') return '22';
...
type PathTarget =
  ...
  | { type: 'classInviteProfile' }
  | { type: 'classInviteJoined' }
...
const invitePathNodes: Array<{ label: string; desc: string; target: PathTarget }> = [
  ...
  { label: '已登录资料不完整', desc: '进入 02B 完善信息', target: { type: 'classInviteProfile' } },
  { label: '已登录已加入', desc: '进入班级列表', target: { type: 'classInviteJoined' } },
  { label: '已登录未加入', desc: '班级列表弹窗', target: { type: 'classInviteConfirm' } },
];
...
const parentPageLabels: Record<ParentPageKey, { number: string; title: string }> = {
  ...
  bindingTotalLimitNotice: { number: '04C', title: '绑定提醒' },
...
const parentFlowLanes: ParentFlowLane[] = [
  {
    ...
    source: 'login',
    branches: [
      { text: '手机号匹配成功&有新增绑定', pages: [{ page: 'bindSelfMatched' }, { page: 'landing' }] },
      { text: '手机号未匹配', pages: [{ page: 'bindSelfUnmatched' }] },
      { text: '手机号匹配成功&无新增绑定', pages: [{ page: 'landing' }] },
    ],
  },
  {
    title: '被邀请',
    ...
    branches: [
      {
        ...
        pages: [{
          page: 'login',
          branches: [
            { text: '手机号匹配成功&有新增绑定', pages: [{ page: 'bindInviteMatched' }, { page: 'landing' }] },
            { text: '手机号匹配成功&无新增绑定', pages: [{ page: 'landing' }] },
            ...
            { text: '已绑定3名学生&继续新增', pages: [{ page: 'bindingTotalLimitNotice' }, { page: 'landing' }] },
          ],
        }],
      },
    ],
  },
  {
    ...
    source: 'landing',
    branches: [
      { text: '点击新增绑定&绑定学生数<3', pages: [{ page: 'bindSelfUnmatched' }] },
      { text: '点击新增绑定&绑定学生数≧3', pages: [{ page: 'bindingTotalLimitNotice' }, { page: 'landing' }] },
    ],
  },
];
...
const parentPrdBlocks: PrdBlock[] = [
  ...
  { type: 'list', items: [
    ...
    '01 后直接分三支：手机号匹配成功&有新增绑定、手机号未匹配、手机号匹配成功&无新增绑定。',
  ...
  { type: 'list', items: [
    ...
    '01 手机号匹配成功后，如有新增绑定进入 03A；如无新增绑定直接进入 05。',
    '03A 绑定学生：仅承接有新增绑定的场景，展示可绑定学生并选择家长身份，多学生时按顺序逐个判断，全部判断后进入 05。',
  ...
  { type: 'list', items: [
    ...
    '03A 只展示确认绑定所需信息；自主登录匹配成功、被邀请且有新增绑定时共用同一页面。',
    ...
    '05 落地页点击新增绑定：关闭切换孩子弹窗后进入 02B；如系统已检索到新的绑定信息，应主动进入 03A，不依赖用户点击新增绑定。',
...
const opsPrdBlocks: PrdBlock[] = [
  ...
  { type: 'list', items: [
    ...
    '必须：在运营端菜单新增个人用户管理。',
    '必须：在运营端菜单新增建议反馈，承接教师端 14A/14B 更多工具提交的反馈。',
    '必须：个人用户管理展示用户、来源、意向学校、转化状态、报告次数、最近活跃和负责人。',
    '必须：建议反馈展示老师、版本、反馈内容、图片数量、时间和处理状态。',
    '应该：支持按学校、老师活跃度、报告生成情况筛选。',
    '不做：不在教师端强行展示推荐学校开通入口。',
  ] },
  { type: 'h2', text: '页面状态' },
  { type: 'list', items: [
    '正常：展示运营工作台、待办、个人用户管理列表和建议反馈列表。',
    '空数据：暂无 C 端老师线索。',
    '加载：表格骨架屏。',
    '网络错误：表格区域展示重试。',
  ] },
...
const schoolAdminPrdBlocks: PrdBlock[] = [
  ...
  { type: 'h2', text: '功能清单' },
  { type: 'list', items: [
    '后台登录支持密码登录、验证码登录和扫码登录三种方式。',
    '顶部提供个人版 / 学校版切换；一个账号可切换到个人空间或多个学校空间。',
    '切换到个人版后，首页“我的应用”不展示“管理后台”入口。',
    '编辑学生信息添加家长手机号时，若该手机号已作为 3 个孩子的家长手机号，则提示不能添加。',
  ] },
];
...
const pageMeta: Record<PageKey, PageMeta> = {
  ...
  profileInvite: {
    title: '完善信息',
    subtitle: '接受班级邀请前补全教师资料。',
    modules: ['页面标题', '邀请摘要', '姓名输入框', '学校输入框', '任教学科选填', '下一步按钮'],
    ctas: [
      { label: '下一步', priority: 'P0', position: '页面底部主按钮' },
    ],
    states: {
      normal: '邀请摘要只展示邀请人和班级，不重复学校名称；学校字段默认带入邀请方学校且允许修改，并支持选填任教学科。',
      loading: '保存中。',
      empty: '姓名或学校为空时下一步按钮不可用。',
      network: '保存失败，保留表单和邀请上下文。',
      denied: '登录失效时返回登录页，重新登录后继续处理邀请。',
    },
  },
  home: {
    title: '新手首页',
  ...
  studentAdd: {
    title: '添加学生',
    subtitle: '手机端录入姓名、必填学号、性别，支持扫码识别表格。',
    ...
    ctas: [
      ...
      { label: '稍后添加', priority: 'P2', position: '页面底部次按钮' },
      { label: '清除该学生', priority: 'P2', position: '多行录入时学生行右侧留白区' },
    ],
    states: {
      normal: '展示一行学生输入，姓名和学号必填，性别默认男，可新增输入行。',
      loading: '保存或导入中。',
      empty: '无学生时引导添加第一名。',
  ...
  classListPersonal: {
    title: '班级列表（个人版）',
    subtitle: '查看个人体验内的班级。',
    modules: ['班级操作入口', '班级显示设置', '班级卡片', '班级名称', '推断年级', '任教标签', '8 位数字班级号', '班级人数'],
    ctas: [
      { label: '班级操作', priority: 'P0', position: '页面右上角' },
      { label: '复制班级号', priority: 'P1', position: '班级卡片班级号' },
  ...
  classActionTeacherPreview: {
    title: '普通老师更多操作',
    subtitle: '个人版与学校版共用，直接查看普通老师可使用的班级操作。',
    modules: ['班级更多操作弹层', '顶部班级详情入口', '日常操作'],
    ctas: [
    ...
    states: {
      normal: '隐藏班级列表背景，默认展开普通老师的班级更多操作。',
      loading: '班级操作加载中。',
  ...
  classActionManagerPreview: {
    title: '班主任/副班主任更多操作',
    subtitle: '个人版与学校版共用，直接查看班级管理角色可使用的班级操作。',
    modules: ['班级更多操作弹层', '顶部班级详情入口', '日常操作', '学生管理', '协同管理'],
    ctas: [
      { label: '作业录入', priority: 'P0', position: '更多操作弹层日常操作' },
      { label: '兑换奖励', priority: 'P0', position: '更多操作弹层日常操作' },
    ...
    states: {
      normal: '隐藏班级列表背景，默认展开班主任/副班主任的班级更多操作。',
      loading: '班级操作加载中。',
      empty: '无可操作班级时不展示更多操作弹层。',
      network: '加载失败，关闭弹层并提供重试。',
  ...
  classDetailMember: {
    title: '班级详情',
    subtitle: '普通老师只读查看班级基础信息、老师和家长绑定情况，并可退出班级。',
  ...
  classDetailDeputy: {
    title: '班级详情',
    subtitle: '副班主任可编辑班级基本信息并邀请老师或家长，不可管理副班主任。',
    modules: ['班级基本信息卡片', '班级名称', '班级学段', '8 位数字班级号', '老师列表卡片', '家长绑定列表卡片', '退出班级'],
    ctas: [
      { label: '编辑班级信息', priority: 'P1', position: '班级名称右侧 icon' },
      { label: '复制班级号', priority: 'P1', position: '班级号右侧 icon' },
      { label: '邀请老师', priority: 'P0', position: '老师列表卡片内' },
      { label: '邀请家长绑定', priority: 'P0', position: '家长绑定列表卡片内' },
      { label: '退出班级', priority: 'P3', position: '页面底部' },
    ],
    states: {
      normal: '展示副班主任视角，可编辑班级并邀请老师或家长，不展示班主任专属管理操作。',
      loading: '班级详情加载中。',
      empty: '老师或家长绑定为空时保留对应邀请入口。',
      network: '加载失败，提供重试。',
      denied: '无班级查看权限时返回班级页。',
    },
  },
  ...
  teacherListDeputy: {
    title: '老师列表',
    subtitle: '副班主任可查看老师列表并邀请老师，不可设置副班主任或移除老师。',
    modules: ['老师列表', '头像', '姓名', '班主任标签', '副班主任标签', '多学科标签'],
    ctas: [
      { label: '邀请老师', priority: 'P0', position: '页面底部固定按钮' },
    ],
    states: {
      normal: '展示全部已加入老师和邀请入口，不展示老师更多操作。',
      loading: '老师列表加载中。',
      empty: '无其他老师时保留邀请老师入口。',
      network: '加载失败，提供重试。',
      denied: '无权限查看时返回班级详情。',
    },
  },
  parentBindingList: {
    title: '家长绑定列表',
    subtitle: '班主任维护当前班级学生家长绑定情况。',
    modules: ['未绑定/已绑定切换', '待绑定', '学生头像', '姓名', '家长身份', '拨打电话', '移除绑定'],
    ctas: [
      { label: '邀请家长绑定', priority: 'P0', position: '页面底部固定按钮' },
    ],
    states: {
      normal: '通过未绑定/已绑定切换查看学生；未绑定学生统一展示待绑定，已绑定学生按学生聚合展示每个家长的身份和操作入口。',
      loading: '绑定情况加载中。',
      empty: '暂无学生。',
      network: '加载失败，提供重试。',
      denied: '无权限查看时返回班级详情。',
  ...
  parentBindingListMember: {
    title: '家长绑定列表',
    subtitle: '非班主任查看当前班级学生家长绑定情况。',
    modules: ['未绑定/已绑定切换', '待绑定', '学生头像', '姓名', '家长身份', '拨打电话'],
    ctas: [],
    states: {
      normal: '通过未绑定/已绑定切换查看学生；未绑定学生统一展示待绑定，已绑定学生可查看绑定关系并拨打电话，不可修改关系、手机号或移除绑定。',
      loading: '绑定情况加载中。',
      empty: '暂无学生。',
      network: '加载失败，提供重试。',
      denied: '无权限查看时返回班级详情。',
  ...
  mineSchool: {
    ...
    states: {
      normal: '展示教师信息、班级来源入口和常用管理入口，不展示了解学校版按钮。',
      loading: '账号信息加载中。',
      empty: '账号信息缺失。',
      network: '加载失败，提供重试。',
      denied: '登录失效，提示重新登录。',
  ...
  suggestionFeedback: {
    title: '建议反馈',
    subtitle: '提交文字和图片给运营端处理。',
    modules: ['反馈内容', '上传图片', '图片数量', '提交按钮'],
    ctas: [
      { label: '提交', priority: 'P0', position: '页面底部主按钮' },
      { label: '上传图片', priority: 'P1', position: '图片上传区' },
    ],
    states: {
      normal: '展示反馈内容输入和 1-5 张图片上传。',
      loading: '提交中。',
      empty: '反馈内容为空时提交按钮不可用。',
      network: '提交失败，保留表单。',
      denied: '登录失效时返回我的页。',
    },
  },
...
const isPersonalClassListPage = (pageKey: PageKey) => pageKey === 'classListPersonal';

const isSchoolClassListPage = (pageKey: PageKey) => pageKey === 'classListSchool';

const isClassActionPreviewPage = (pageKey: PageKey) => (
  pageKey === 'classActionTeacherPreview' || pageKey === 'classActionManagerPreview'
);
...
const getClassRoleForPage = (pageKey: PageKey): ClassRole => {
  if (pageKey === 'classDetail' || pageKey === 'classDetailSchoolHead' || pageKey === 'teacherList') return 'headTeacher';
  if (pageKey === 'classDetailDeputy' || pageKey === 'teacherListDeputy' || pageKey === 'parentBindingList') return 'deputyHeadTeacher';
  return 'teacher';
};
...
const classDetailComparisonPrdBlocks: PrdBlock[] = [
  ...
  { type: 'list', items: [
    ...
    '08B 普通老师：任课老师或被邀请加入的普通协作老师，只能查看班级资料、老师列表和家长绑定情况，不能编辑班级、邀请成员或管理老师角色。',
    '08C 学校版班主任：学校统一管理下的班主任，允许维护班级协作和转移班主任，但班级归属学校，不允许在手机端解散。',
    '08D 副班主任：个人版和学校版共用，可编辑班级基本信息并邀请老师或家长；不能设置副班主任、移除老师或转移班主任。',
  ] },
  { type: 'h2', text: '进入条件' },
  { type: 'list', items: [
    '从 07A 个人版班级列表进入时，班主任进入 08A，副班主任进入 08D，普通老师进入 08B。',
    '从 07B 学校版班级列表进入时，班主任进入 08C，副班主任进入 08D，普通老师进入 08B。',
    '被邀请老师已加入班级后再次打开邀请卡片，按班级角色和当前版本归属落到 08A、08B、08C 或 08D，不新增邀请承接页。',
  ] },
  { type: 'h2', text: '核心差异' },
  { type: 'list', items: [
    '编辑班级信息：08A、08C、08D 可以编辑；08B 不展示编辑入口。',
    '邀请权限：08A、08C、08D 可以邀请老师和家长；08B 不展示任何邀请入口。',
    '老师角色管理：08A、08C 进入 09A，可设置副班主任、移除老师、转移班主任；08D 进入 09C，只查看和邀请；08B 进入 09B，只查看。',
    '家长绑定：08A、08C、08D 进入 10A，可维护并邀请家长；08B 进入 10B，只读查看。',
    '危险操作：08A 是解散班级；08B、08C、08D 是退出班级。',
  ] },
  { type: 'h2', text: '确认规则' },
  { type: 'list', items: [
  ...
  { type: 'h2', text: '设计取舍' },
  { type: 'list', items: [
    '08A/08B/08C/08D 保持同一信息骨架，是为了让老师切换身份时认知成本最低；差异只体现在权限和危险操作。',
    '不把所有角色差异都拆成页面，是为了避免页面地图变复杂；只有会影响关键权限或学校/个人版归属的状态才独立编号。',
    '普通老师的无权限操作直接隐藏，不使用禁用按钮或额外权限说明占用真实页面空间。',
  ] },
];
...
const pagePrdDetails: Partial<Record<PageKey, PrdBlock[]>> = {
  login: [
    ...
    { type: 'list', items: [
      ...
      '新用户首次登录时进入 02A 完善信息 → 03 新手首页。',
      '新用户已填写姓名但未完成新手引导时直接进入 03 新手首页，后续节点不重复展示。',
      '老用户登录时直接进入 13 记录页。',
      '空间选择不作为首次进入流程中的独立页面节点。',
      '已登录老师再次进入时，系统按上次使用空间直接进入对应首页。',
      '个人版在账号注册成功时自动创建；从邀请入口注册时后台创建但不打断受邀流程。',
    ] },
    { type: 'h2', text: '邀请入口未登录' },
    { type: 'list', items: [
      '老师从微信小程序卡片或邀请链接进入时，如果未登录，直接进入 01 登录页。',
      '登录页不展示额外邀请承接内容，保持登录主流程简洁。',
      '完成微信或手机号授权后，先校验教师资料是否完整，再校验是否已加入被邀请班级。',
      '资料不完整时进入 02B 受邀完善信息，资料完整后再进入加入确认。',
      '如果已加入该班级，进入邀请方学校的班级列表；如果未加入该班级，进入同一列表并展示加入确认弹窗。',
      '班级邀请只授予目标班级权限，不自动授予邀请方学校的全校权限。',
    ] },
  ],
  minePersonal: [
  ...
  mineSchool: [
    ...
    { type: 'list', items: [
      ...
      '教师卡片头像姓名下方展示班级来源胶囊。点击后从底部上滑展示“切换班级来源”。',
      '切换班级来源弹窗只展示来源名称、个人/协作/学校标签和当前标签，不展示“个人体验数据”“学校统一数据”等备注文案。',
  ...
  classListPersonal: [
    ...
    { type: 'list', items: [
      ...
      '点击发送后完成转发，小程序卡片打开后按登录态和资料状态进入登录页、02B 受邀完善信息或班级列表。',
      '被邀请方先在微信聊天对话中看到小程序卡片，点击卡片后再进入小程序。',
      '选择链接邀请时只展示可复制文案，文案包含班级名称和快速加入链接。',
      '邀请链接打开后不出现额外承接页：未登录进登录页，资料不完整进 02B，资料完整后进入邀请方学校的班级列表。',
      '已登录但没有该班级时，邀请方学校的班级列表展示加入确认弹窗。',
      '右侧页面总览下方的“被邀请”用于展示被邀请方路径，从微信聊天对话收到小程序卡片开始。',
    ] },
    { type: 'h2', text: '被邀请方路由' },
    { type: 'list', items: [
      '已登录且已有该班级：直接进入邀请方学校的班级列表，不展示加入确认。',
      '已登录但没有该班级：进入邀请方学校的班级列表，并在列表上展示加入确认弹窗。',
      '确认弹窗说明邀请老师、班级名称和加入后可共同管理学生日常评价。',
      '点击同意加入后关闭弹窗并停留在邀请方学校的班级列表；点击暂不加入后进入个人版新手首页。',
    ] },
  ],
  classActionTeacherPreview: [
    { type: 'h2', text: '07C 普通老师更多操作' },
    { type: 'list', items: [
      '个人版和学校版使用同一套普通老师菜单，因此合并为一个共享验收状态。',
      '进入 07C 时隐藏班级列表背景，只展示目标班级的更多操作弹层。',
      '普通老师不展示批量修改学生、更新人脸数据、设置兑换密码、邀请老师加入、邀请家长加入和离校学生管理。',
      '点击弹层顶部班级名称行进入 08B 普通老师只读班级详情。',
    ] },
  ],
  classActionManagerPreview: [
    { type: 'h2', text: '07D 班主任/副班主任更多操作' },
    { type: 'list', items: [
      '个人版和学校版使用同一套班主任/副班主任菜单，因此合并为一个共享验收状态。',
      '进入 07D 时隐藏班级列表背景，并以副班主任身份默认展开更多操作；班主任在该菜单中的能力相同。',
      '依次展示日常操作、学生管理和协同管理；离校学生管理归入学生管理。',
      '点击弹层顶部班级名称行进入 08D；班主任从真实班级列表进入时再按版本进入 08A 或 08C。',
    ] },
  ],
  classListSchool: [
    { type: 'h2', text: '班级卡片结构' },
  ...
  classDetail: [
    ...
    { type: 'list', items: [
      ...
      '完整家长绑定列表不展示学号；未绑定卡片展示学生头像、学生姓名和“待绑定”标签。',
      '已绑定预览按学生聚合展示，例如“王小明”卡片内展示“爸爸、姑姑”等关系标签；08A/08B 预览不展示电话 icon。',
      '班主任和副班主任视角的班级详情、完整家长绑定列表均提供“邀请家长绑定”按钮，普通老师视角不展示。',
  ...
  classDetailMember: [
    ...
    { type: 'h2', text: '普通老师补充' },
    { type: 'list', items: [
      '从 07A/07B 班级列表进入普通老师身份的班级时，跳转到 08B 班级详情。',
      '普通老师可以查看班级基础信息、复制班级号、老师列表和家长绑定情况，但不能修改班级详情，也不能邀请老师或家长。',
      '班级卡片沿用班主任详情结构：不展示班主任字段，班级号和人数同一行。',
      '页面底部不展示解散班级，只展示退出班级。',
      '退出班级需要二次确认；确认后当前老师不再管理该班级，班级数据仍由班主任保留。',
  ...
  classDetailDeputy: [
    ...
    { type: 'h2', text: '副班主任补充' },
    { type: 'list', items: [
      '从 07A/07B 班级列表进入副班主任身份的班级时，跳转到 08D 班级详情。',
      '副班主任可以编辑班级基本信息、邀请老师和邀请家长。',
      '副班主任进入 09C 老师列表，可查看并邀请老师，但不能设置或取消副班主任、移除老师、转移班主任。',
      '副班主任进入 10A 家长绑定列表，可沿用现有家长绑定维护权限。',
      '页面底部只展示退出班级，不展示解散班级或转移班主任。',
    ] },
  ],
  teacherList: [
  ...
  teacherListMember: [
    ...
    { type: 'list', items: [
      '09B 是普通老师视角。',
      '老师卡片包含头像、姓名、班主任标签、副班主任标签和多个任教学科标签。',
      '普通老师只可查看老师列表；班主任和副班主任置顶并用横线分隔其余老师，不展示邀请、移除或更多操作。',
    ] },
  ],
  teacherListDeputy: [
    { type: 'h2', text: '09C 老师列表规则' },
    { type: 'list', items: [
      '09C 是副班主任视角。',
      '老师卡片与 09A/09B 使用同一信息结构，班主任和副班主任置顶。',
      '副班主任可邀请老师，邀请按钮固定在页面底部。',
      '副班主任不展示老师卡片更多操作，不能设置或取消副班主任，也不能移除老师。',
    ] },
  ],
  parentBindingList: [
    { type: 'h2', text: '10A 家长绑定列表规则' },
    { type: 'list', items: [
      '10A 是班主任视角，班主任和副班主任可以维护家长绑定关系。',
      '页面使用“未绑定/已绑定”tab 切换查看学生，tab 内数字即统计。',
      '邀请家长绑定按钮固定在页面底部，与 09 老师列表保持一致。',
      'mock 数据为 48/50：50 个学生中 48 个学生已绑定，未绑定学生优先展示。',
      '未绑定卡片展示学生头像、学生姓名和“待绑定”标签。',
      '待绑定统一表示该学生还没有完成家长绑定，不在列表层区分是否已预留手机号。',
      '已绑定 tab 每个家长绑定关系展示为一张可点击卡片，卡片只展示学生头像和“学生姓名+关系”，例如“王小明的爸爸”；姓名字号应克制，关系作为轻层级信息展示。',
      '同一个学生有多个家长绑定时，多个家长卡片连续排列；不同学生之间用弱化线条分隔。',
      '已绑定卡片列表态不展示手机号、不展示 icon 操作；点击卡片后在弹窗中默认展示关系和完整手机号，并只突出拨打电话主按钮。',
      '10A 详情弹窗提供“编辑”入口；点击编辑后才激活关系和手机号输入框，底部只展示左侧移除绑定和右侧保存修改，不再展示拨打电话。',
  ...
  parentBindingListMember: [
    { type: 'h2', text: '10B 家长绑定列表规则' },
    { type: 'list', items: [
      '10B 是非班主任视角。',
      '页面使用“未绑定/已绑定”tab 切换查看学生，tab 内数字即统计。',
      '10B 可查看学生家长绑定关系和完整手机号，并可拨打电话。',
      '10B 不能修改家长绑定关系，不能修改手机号，不能移除绑定。',
      '10B 不展示邀请家长绑定按钮。',
    ] },
  ],
  studentList: [
  ...
  studentAdd: [
    ...
    { type: 'list', items: [
      ...
      '每行包含姓名、较短学号输入框、男/女点选。',
      '性别默认男，且每行可独立切换。',
      '每一行的姓名、学号和性别填写完成后，底部完成按钮才可点击；学号必填。',
      '学号必填，填写后可支持学号进行评价，例如“1号和3号同学打架”。',
      '当存在多行时，每个学生行右侧留白区展示清除 icon，用于删除误添加的整条学生数据；不展示学生序号文案。',
      '底部提供“稍后添加”次按钮，点击后不保存当前未完成的学生行，返回首页并保持“确认学生”任务未完成；老师后续可再次进入本页使用扫码识别表格。',
      '不提供批量粘贴入口。',
    ] },
  ],
  termManagement: [
...
const getTeacherSpaceDisplayName = (space: TeacherSpaceProfile) => {
  if (space.type === 'school') return space.title;
...
const TeacherCMobileLowFi: React.FC = () => {
  ...
  const [userHasMultipleClassSources, setUserHasMultipleClassSources] = useState(true);
  ...
  const [showSchoolNameSheet, setShowSchoolNameSheet] = useState(false);
  const [draftTeacherSchoolName, setDraftTeacherSchoolName] = useState('');
  ...
  const [suggestionFeedbackText, setSuggestionFeedbackText] = useState('');
  const [suggestionFeedbackImages, setSuggestionFeedbackImages] = useState<string[]>([]);
  const [teacherSuggestionFeedbackRows, setTeacherSuggestionFeedbackRows] = useState<TeacherSuggestionFeedback[]>([
    { id: 'feedback-1', teacher: '李明老师', phone: '138****2468', version: '个人版', content: '希望批量导入学生时能自动识别学号列。', imageCount: 2, time: '10:18', status: '待处理' },
    { id: 'feedback-2', teacher: '郭老师', phone: '139****8021', version: '学校版', content: '建议期末报告生成后支持按班级查看失败原因。', imageCount: 1, time: '昨天', status: '已处理' },
  ]);
  ...
  const [schoolAdminLoginTab, setSchoolAdminLoginTab] = useState<SchoolAdminLoginTab>('scan');
  const [schoolAdminLoggedIn, setSchoolAdminLoggedIn] = useState(false);
  const [opsPage, setOpsPage] = useState<'home' | 'personalUsers' | 'suggestionFeedback'>('home');
  ...
  const [inviteClass, setInviteClass] = useState<{ name: string; code: string; inviter: string; school: string }>({
    name: '2025级1班',
    code: '58273914',
    inviter: '郭老师',
    school: '成都七中初中附属小学',
  ...
  const [classDissolveStep, setClassDissolveStep] = useState<ClassDissolveStep>('check');
  ...
  const [isEditingParentBinding, setIsEditingParentBinding] = useState(false);
  ...
  const [classHeadTeacherId, setClassHeadTeacherId] = useState('teacher-current');
  ...
  const copyClassCode = async (code: string) => {
    try {
      ...
      showClassActionToast('班级号已复制');
    } catch {
      showClassActionToast('复制失败，请重试');
    }
  };
  ...
  const classTeachers: ClassTeacherProfile[] = [
    { id: 'teacher-current', name: currentTeacherDisplayName, subjects: inviteTeachingSubjects.length > 0 ? inviteTeachingSubjects : ['语文', '道德与法治'], isHeadTeacher: false, isDeputyHeadTeacher: false },
    { id: 'teacher-chen', name: '陈老师', subjects: ['语文', '书法'], isHeadTeacher: false, isDeputyHeadTeacher: true },
    { id: 'teacher-li', name: '李老师', subjects: ['数学', '科学'], isHeadTeacher: false, isDeputyHeadTeacher: false },
    { id: 'teacher-wang', name: '王老师', subjects: ['英语'], isHeadTeacher: false, isDeputyHeadTeacher: false },
    { id: 'teacher-zhao', name: '赵老师', subjects: ['体育', '劳动'], isHeadTeacher: false, isDeputyHeadTeacher: true },
    { id: 'teacher-liu', name: '刘老师', subjects: ['美术'], isHeadTeacher: false, isDeputyHeadTeacher: false },
    { id: 'teacher-ma', name: '马老师', subjects: ['信息科技'], isHeadTeacher: false, isDeputyHeadTeacher: false },
    { id: 'teacher-gao', name: '高老师', subjects: ['综合实践'], isHeadTeacher: false, isDeputyHeadTeacher: false },
  ].map((teacher) => ({
    ...teacher,
    isHeadTeacher: teacher.id === classHeadTeacherId,
  }));
  ...
  const startParentBindingLookup = () => {
    setParentBindingReturnPage('landing');
    if (parentBindingLookupTimerRef.current !== null) {
      window.clearTimeout(parentBindingLookupTimerRef.current);
    }
    jumpToParentPage('bindSelfUnmatched');
  };
  ...
  const jumpToPage = (next: PageKey) => {
    ...
    setClassActionToast('');
    setHistory([]);
    if (isClassActionPreviewPage(next)) {
      const isOrdinaryTeacherPreview = next === 'classActionTeacherPreview';
    ...
    if (next === 'classDetailMember') {
      setHasOtherClassMemberships(true);
      setActiveClassProfile({
        name: '2024级2班',
        code: '73948162',
        stage: '初中',
    ...
    if (next === 'minePersonal') {
      setCurrentSpaceId('personal');
      setUserHasMultipleClassSources(false);
    }
    if (next === 'mineSchool') {
      setCurrentSpaceId(defaultTeacherSpaceId);
    ...
    if (!teacherName.trim()) setTeacherName('郭');
    if (next !== 'profile' && next !== 'profileInvite') setClassCreated(true);
  ...
  const getInvitedSchoolSpaceId = (): TeacherSpaceId => (
    teacherSpaces.find((space) => space.type === 'school' && space.title === inviteClass.school)?.id
      ?? defaultTeacherSpaceId
  );

  const openInvitedClassList = (showJoinConfirm = false) => {
    setCurrentSpaceId(getInvitedSchoolSpaceId());
    navigate('classListSchool');
    if (showJoinConfirm) setShowInviteConfirmSheet(true);
  };
  ...
  const jumpToPath = (target: PathTarget) => {
    ...
    if (target.type === 'classInviteProfile') {
      jumpToPage('profileInvite');
      setInviteeLoggedIn(true);
      setPendingInviteAfterLogin(true);
      setTeacherName('');
      setTeacherSchoolName(inviteClass.school);
      setInviteTeachingSubjects([]);
      return;
    }

    if (target.type === 'classInviteJoined') {
      setInviteeLoggedIn(true);
      setInviteeAlreadyJoined(true);
      setHasOtherClassMemberships(false);
      openInvitedClassList();
      return;
    }

    if (target.type === 'classInviteConfirm') {
      setInviteeLoggedIn(true);
      setInviteeAlreadyJoined(false);
      setHasOtherClassMemberships(false);
      openInvitedClassList(true);
      return;
    }
  ...
  const getClassDetailPageForProfile = (profile: ClassProfile): PageKey => {
    if (profile.role === 'deputyHeadTeacher') return 'classDetailDeputy';
    if (profile.role === 'teacher') return 'classDetailMember';
    return getClassListPageForCurrentSpace() === 'classListSchool' ? 'classDetailSchoolHead' : 'classDetail';
  };
  ...
  const getClassListPageForSpace = (spaceId: TeacherSpaceId): PageKey => {
    const targetSpace = teacherSpaces.find((space) => space.id === spaceId) ?? teacherSpaces[0];
    return targetSpace.type === 'school' ? 'classListSchool' : 'classListPersonal';
  };
  ...
  const jumpToParentPage = (next: ParentPageKey, directoryNodeKey?: string) => {
    ...
    if ((next === 'bindSelfMatched' || next === 'bindSelfUnmatched' || next === 'bindInviteMatched' || next === 'bindInviteUnmatched' || next === 'bindInviteBoundUnmatched' || next === 'bindingLimitNotice' || next === 'bindingTotalLimitNotice' || next === 'bindingNotice') && directoryNodeKey) {
  ...
  const completeLogin = () => {
    ...
    setInviteeLoggedIn(true);
    if (!teacherName.trim() || !teacherSchoolName.trim()) {
      setTeacherSchoolName((schoolName) => schoolName.trim() || inviteClass.school);
      navigate('profileInvite');
      return;
    }
  ...
  const fillStudentsFromScannedTable = () => {
    setStudentInputRows([
      ...
      { id: 2, name: '周雨晴', no: '20250102', gender: '女' },
      { id: 3, name: '陈浩然', no: '20250103', gender: '男' },
    ]);
    setShowStudentScanSheet(false);
  };

  const addSuggestionFeedbackImage = () => {
    setSuggestionFeedbackImages((images) => images.length >= 5 ? images : [...images, `图片${images.length + 1}`]);
  };

  const removeSuggestionFeedbackImage = (index: number) => {
    setSuggestionFeedbackImages((images) => images.filter((_, imageIndex) => imageIndex !== index));
  };

  const submitSuggestionFeedback = () => {
    const text = suggestionFeedbackText.trim();
    if (!text) return;
    const version: TeacherSuggestionFeedback['version'] = currentSpaceId === 'personal' ? '个人版' : '学校版';
    setTeacherSuggestionFeedbackRows((rows) => [{
      id: `feedback-${Date.now()}`,
      teacher: '郭老师',
      phone: '139****8021',
      version,
      content: text,
      imageCount: suggestionFeedbackImages.length,
      time: '刚刚',
      status: '待处理',
    }, ...rows]);
    setSuggestionFeedbackText('');
    setSuggestionFeedbackImages([]);
    showClassActionToast('建议反馈已提交');
    navigate(currentSpaceId === 'personal' ? 'minePersonal' : 'mineSchool');
  };

  const markSuggestionFeedbackStatus = (id: string, status: TeacherSuggestionFeedback['status']) => {
    setTeacherSuggestionFeedbackRows((rows) => rows.map((row) => row.id === id ? { ...row, status } : row));
  };
  ...
  const submitSchoolNameSheet = () => {
    const nextSchoolName = draftTeacherSchoolName.trim();
    if (!nextSchoolName) return;
    setTeacherSchoolName(nextSchoolName);
    setShowSchoolNameSheet(false);
  };
  ...
  const ScreenHeader = ({ title, sub, onBack, backLabel = '返回' }: { title: string; sub?: string; onBack?: () => void; backLabel?: string }) => {
    const backAction = onBack ?? (history.length ? back : undefined);
    return (
      <header className="flex h-16 items-center gap-3 border-b border-gray-100 bg-white px-4">
        <button type="button" onClick={backAction} className="flex h-11 w-11 items-center justify-center rounded-xl active:bg-gray-100 disabled:opacity-30" disabled={!backAction} aria-label={backLabel}>
          <ArrowLeft size={18} />
        </button>
  ...
  const BottomNav = () => (
    <nav className="absolute inset-x-4 bottom-4 grid h-14 grid-cols-3 rounded-2xl border border-gray-100 bg-white shadow-[0_-8px_28px_rgba(15,23,42,0.06)] text-center text-[11px] font-black">
      ...
      <button type="button" onClick={() => navigate(getClassListPageForCurrentSpace())} className={cx((isPersonalClassListPage(page) || isSchoolClassListPage(page)) && 'bg-gray-950 text-white')}>班级</button>
      <button type="button" onClick={() => navigate(userHasMultipleClassSources ? 'mineSchool' : 'minePersonal')} className={cx('rounded-r-[14px]', (page === 'minePersonal' || page === 'mineSchool') && 'bg-gray-950 text-white')}>我的</button>
    </nav>
  );
  ...
  const runClassAction = (key: ClassActionKey) => {
    const permissions = classRolePermissions[activeClassProfile.role];
    ...
    if (key === 'inviteTeacher' || key === 'inviteParent') {
      const canInvite = key === 'inviteTeacher' ? permissions.canInviteTeacher : permissions.canInviteParent;
      if (!canInvite) return;
  ...
  const ClassCard = ({ name, code, stage, entryYearValue, tags, count, creatorName, isCreator, role }: { name: string; code: string; stage: '小学' | '初中' | '高中'; entryYearValue: number; tags: string[]; count: number; creatorName: string; isCreator: boolean; role: ClassRole }) => {
    ...
    <div className="relative rounded-2xl bg-gray-50 p-4">
      ...
      <div className="mt-1 flex min-h-11 items-center justify-between gap-3 text-xs font-medium text-gray-600">
        <button
          type="button"
  ...
  const renderClassActionSheet = () => {
    ...
    return (
      <div className={cx(bottomSheetBackdropClass, 'z-40 bg-black/35')} role="dialog" aria-modal="true" aria-label="班级更多操作">
  ...
  const renderTeacherActionSheet = () => {
    if (!activeTeacherAction || !classRolePermissions[getClassRoleForPage(page)].canManageDeputy) return null;
  ...
  const renderParentBindingDetailSheet = () => {
    ...
    const close = () => {
      setActiveParentBindingCandidate(null);
      setIsEditingParentBinding(false);
    };
    const canManageParentBinding = page === 'parentBindingList';
    const showParentBindingEditFields = canManageParentBinding && isEditingParentBinding;
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          {showParentBindingEditFields ? (
            ...
            <div className="space-y-2 rounded-2xl bg-gray-50 p-3">
              <div className="flex h-11 items-center justify-between rounded-xl bg-white px-3">
                <span className="text-xs font-black text-gray-500">关系</span>
                <span className="text-sm font-semibold text-gray-950">{currentGuardianLabel}</span>
              </div>
              <div className="flex h-11 items-center justify-between rounded-xl bg-white px-3">
                <span className="text-xs font-black text-gray-500">手机号</span>
                <span className="text-sm font-semibold tabular-nums text-gray-950">{activeParentBindingCandidate.phone}</span>
              </div>
            </div>
          )}
          {showParentBindingEditFields ? (
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button
                type="button"
                disabled={!canRemoveParentBinding}
                onClick={() => {
                ...
                className={cx(
                  'flex h-12 items-center justify-center gap-2 rounded-2xl border border-gray-200 text-sm font-black',
                  canRemoveParentBinding ? 'bg-white text-red-600 active:bg-red-50' : 'bg-gray-100 text-gray-400'
                )}
              >
                <Trash2 size={16} />
                移除绑定
              ...
              <button
                ...
                className={cx(
                  ...
                  canSaveParentBinding ? 'bg-gray-900 text-white active:bg-gray-800' : 'bg-gray-100 text-gray-400'
                )}
              >
                保存修改
              </button>
            </div>
          ) : canManageParentBinding && (
            <>
              <button
                type="button"
                onClick={() => showClassActionToast(`正在拨打${activeParentBindingCandidate.studentName}${currentGuardianLabel}电话`)}
                className="mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-gray-900 text-sm font-black text-white active:bg-gray-800"
              >
                <Phone size={16} />
                拨打电话
              </button>
              <button
                type="button"
                onClick={() => setIsEditingParentBinding(true)}
                className="mt-2 flex h-12 w-full items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-white text-sm font-black text-gray-950 active:bg-gray-50"
              >
                <Edit3 size={16} />
                编辑
              </button>
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                setClassHeadTeacherId(transferHeadTeacherCandidate.id);
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          <div className="mt-4 space-y-2">
            ...
            >
              ...
              <span className="text-sm font-black">通过链接邀请</span>
            </button>
          </div>
      ...
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="二维码邀请">
        ...
        <section className={cx(bottomSheetPanelClass, 'max-h-[92%] overflow-y-auto rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          <div className="mt-4 overflow-hidden rounded-3xl border border-gray-200 bg-white p-5 text-center">
            <div className="text-lg font-black text-gray-950">{qrInviteTitle}</div>
            <img src={qrInviteImage} alt={qrInviteAlt} className="mx-auto mt-5 h-36 w-36 rounded-2xl border border-gray-100 bg-gray-50 object-cover p-2" />
  ...
  const renderInviteConfirmSheet = () => {
    ...
    const declineInvite = () => {
      close();
      if (inviteAudience !== 'teacher') return;
      setCurrentSpaceId('personal');
      setUserHasMultipleClassSources(false);
      setClassCreated(false);
      setJoinedClassHasStudents(false);
    ...
    return (
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="确认加入班级">
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            <button type="button" onClick={declineInvite} className="h-12 rounded-2xl border border-gray-200 bg-white text-sm font-black active:bg-gray-100">暂不加入</button>
            <button
              type="button"
              onClick={() => {
                close();
  ...
  const renderClassExitSheet = () => {
    ...
    const dissolvePrimaryLabel = classDissolveStep === 'check' ? '已确认，继续' : '确认解散班级';
    const dissolveTitle = classDissolveStep === 'check' ? '解散班级' : '最终确认';
    ...
    return (
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label={isDissolveMode ? '确认解散班级' : '确认退出班级'}>
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          <div className="mx-auto mb-4 h-1.5 w-12 rounded-full bg-gray-300" aria-hidden="true" />
          <h3 className="text-base font-black">{isDissolveMode ? dissolveTitle : '退出班级'}</h3>
          {isDissolveMode ? (
            <div className="mt-4 space-y-3">
              {classDissolveStep === 'check' && (
                <>
                  ...
                  <ul className="mt-2 space-y-1.5 text-xs font-medium leading-5 text-gray-600">
                    <li>没有其他老师加入该班级</li>
                    <li>没有任何家长绑定该班级学生</li>
                  </ul>
                  </section>
              ...
              {classDissolveStep === 'final' && (
                <>
                  <section className="rounded-2xl border border-red-200 bg-red-50 p-3">
                    <div className="text-sm font-black text-red-600">解散后将清空该班级信息和所有学生信息，且不可恢复。</div>
                  </section>
                  <label className="block text-xs font-black text-gray-500">
                    输入英文 delete 确认解散
                    <input
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              disabled={isDissolveMode && classDissolveStep === 'final' && !canDissolve}
              onClick={() => {
                if (isDissolveMode && classDissolveStep === 'check') {
                  setClassDissolveStep('final');
                  return;
                }
                close();
                showClassActionToast(isDissolveMode ? '已解散班级' : '已退出班级');
                if (!isDissolveMode && !hasOtherClassMemberships) {
                  setInviteeAlreadyJoined(false);
                  setCurrentSpaceId('personal');
                  setUserHasMultipleClassSources(false);
                  setClassCreated(false);
              ...
              className={cx(
                'h-12 rounded-2xl border border-gray-200 text-sm font-black',
                isDissolveMode && classDissolveStep === 'final' && !canDissolve ? 'bg-gray-100 text-gray-400' : 'bg-gray-900 text-white'
              )}
            >
              {isDissolveMode ? dissolvePrimaryLabel : '确认退出'}
            </button>
          </div>
        </section>
      </div>
  ...
  const TeacherRoleTags = ({ teacher }: { teacher: ClassTeacherProfile }) => (
    <div className="mt-2 flex flex-wrap gap-1.5">
      ...
      {teacher.subjects.map((subject) => (
        <span key={`${teacher.id}-${subject}`} className="rounded-lg border border-gray-200 bg-white px-2 py-0.5 text-[11px] font-bold text-gray-600">{subject}</span>
      ))}
    </div>
  );
  ...
  const canRemoveParentBinding = activeClassProfile.isCreator || classTeachers.some((teacher) => teacher.id === 'teacher-current' && (teacher.isHeadTeacher || teacher.isDeputyHeadTeacher));
  ...
  const ParentBindingGuardianCard = ({ student, guardian }: { student: ParentBindingProfile; guardian: ParentBindingProfile['guardians'][number] }) => {
    ...
    return (
      <button
        ...
        onClick={() => {
          setIsEditingParentBinding(false);
          setActiveParentBindingCandidate({
            ...
            {lane.title === '登录' ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="grid shrink-0 grid-cols-2 gap-2">
                  ...
                  <PageNodeButton item="suggestionFeedback" lane={lane.title} />
                </div>
              </div>
            ) : lane.title === '我的(学校版)' ? (
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                {lane.pages.map((item) => <PageNodeButton key={item} item={item} lane={lane.title} />)}
              </div>
            ) : lane.branchGroups ? (
              <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="grid shrink-0 gap-2">
              {([
                ...
                { cond: '已登录资料不完整', node: invitePathNodes[2], idx: 2 },
                { cond: '已登录已加入', node: invitePathNodes[3], idx: 3 },
                { cond: '已登录未加入', node: invitePathNodes[4], idx: 4 },
              ] as const).map((branch) => {
                const destActive = activeInviteNode === branch.idx;
                const destPage = branch.node.target.type === 'page'
                  ? branch.node.target.page
                  : branch.node.target.type === 'classInviteProfile'
                    ? 'profileInvite'
                    : 'classListSchool';
                return (
          ...
          <>
            ...
            <div className="space-y-2 pt-3">
              <button
                ...
                onClick={() => {
                  if (parentIdentityComplete) {
                    if (parentIdentityForm.phone === '17700003333') {
                      jumpToParentPage('bindingTotalLimitNotice');
                      return;
                    }
        ...
        <div className="flex-1 space-y-3 overflow-y-auto">
          <section className="rounded-2xl bg-gray-50 p-4">
            ...
            <p className="mt-2 text-sm font-semibold leading-6 text-gray-700">
              为保障学生信息安全，同一班级最多绑定 2 名学生，一个家长最多绑定 3 名学生。请确认当前绑定均为自己的孩子。
            </p>
          </section>
    ...
    <div className="flex h-full w-full flex-col bg-white text-gray-950">
      ...
      <div className="flex h-[calc(100%-64px)] flex-col px-5 pb-5 pt-5">
        <div className="flex-1 space-y-3 overflow-y-auto">
          ...
          <div className="rounded-2xl bg-gray-50 px-4 py-3 text-sm font-semibold leading-6 text-gray-700">
            一个家长最多绑定 3 名学生。
          </div>
        </div>
        <button
  ...
  const renderParentBindingTotalLimitNotice = () => (
    <div className="flex h-full w-full flex-col bg-white text-gray-950">
      <header className="grid h-16 items-center border-b border-gray-100 bg-white px-4 [grid-template-columns:44px_1fr_44px]">
        <button type="button" onClick={() => jumpToParentPage('landing')} className="flex h-11 w-11 items-center justify-center rounded-xl active:bg-gray-100" aria-label="返回首页">
          <ArrowLeft size={19} />
      ...
      <div className="flex h-[calc(100%-64px)] flex-col px-5 pb-5 pt-5">
        <div className="flex-1 space-y-3 overflow-y-auto">
          <section className="rounded-2xl bg-gray-50 p-4">
            <h3 className="text-xs font-black text-gray-500">安全提醒</h3>
            <p className="mt-2 text-sm font-semibold leading-6 text-gray-700">
              为保障学生信息安全，一个家长最多绑定 3 名学生。当前账号已绑定 3 个孩子，如需调整，请联系班主任先解绑后再重新绑定。
            </p>
          </section>

          {renderParentStudentCard('郑小磊')}
          {renderParentStudentCard('郑小雅')}
          {renderParentStudentCard('郑小宇')}
        </div>
        <button
          type="button"
          onClick={() => jumpToParentPage('landing')}
    ...
    <div className="relative flex h-full w-full flex-col overflow-hidden bg-white text-gray-950">
      <div className="relative flex-1 overflow-y-auto pb-24 pt-5">
        ...
        <section className="mx-5 mt-4 flex flex-col gap-3">
          ...
          <article className="min-h-[154px] rounded-3xl bg-gray-50 p-4">
            <div className="text-[13px] font-black text-gray-500">预计可得</div>
  ...
  const renderParentWechatPhoneSheet = () => {
    ...
    const allowPhone = (phone: string, result: 'matchedWithNewBinding' | 'matchedWithoutNewBinding' | 'unmatchedLimit' | 'totalLimit' = 'matchedWithNewBinding') => {
      const isInviteLogin = parentLoginTarget === 'bindInviteMatched';
      const target: ParentPageKey = result === 'matchedWithNewBinding'
        ? parentLoginTarget
        : result === 'matchedWithoutNewBinding'
          ? 'landing'
          : result === 'totalLimit'
            ? 'bindingTotalLimitNotice'
            : isInviteLogin ? 'bindingLimitNotice' : 'bindSelfUnmatched';
      ...
      if (isInviteLogin) setParentBindMode('class');
      ...
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="微信手机号授权">
        ...
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          ...
          <div className="mt-5 overflow-hidden rounded-2xl bg-gray-50">
            <button type="button" onClick={() => allowPhone('15200001332', 'matchedWithNewBinding')} className="block min-h-16 w-full border-b border-gray-100 px-4 py-3 text-center active:bg-slate-100">
              <div className="text-lg font-medium">152****1332</div>
              <div className="mt-1 text-xs font-black text-slate-600">上次提供</div>
            </button>
            <button type="button" onClick={() => allowPhone('18800005566', 'matchedWithoutNewBinding')} className="block min-h-16 w-full border-b border-gray-100 px-4 py-3 text-center active:bg-slate-100">
              <div className="text-lg font-medium">188****5566</div>
            </button>
            <button type="button" onClick={() => allowPhone('19900008610', 'unmatchedLimit')} className="block min-h-16 w-full px-4 py-3 text-center active:bg-slate-100">
              <div className="text-lg font-medium">199****8610</div>
            </button>
            <button type="button" onClick={() => allowPhone('17700003333', 'totalLimit')} className="block min-h-16 w-full border-t border-gray-100 px-4 py-3 text-center active:bg-slate-100">
              <div className="text-lg font-medium">177****3333</div>
            </button>
          </div>
  ...
  const renderParentPrototype = () => (
    <PhoneMockup showDeviceFrame={false} safeAreaTop={false}>
      <div className="relative h-full w-full overflow-hidden bg-white">
        ...
        {parentPage === 'bindingTotalLimitNotice' && renderParentBindingTotalLimitNotice()}
        ...
        <div className="flex h-[calc(100%-57px)]">
          <aside className="w-[148px] shrink-0 border-r border-gray-200 bg-gray-50 p-3">
            {[
              ...
              { key: 'suggestionFeedback' as const, label: '建议反馈' },
            ].map((item) => {
          ...
          <main className="min-w-0 flex-1 overflow-y-auto p-4">
            ...
            {opsPage === 'suggestionFeedback' && (
              <div className="space-y-3">
                <div className="grid grid-cols-[1fr_0.8fr_0.8fr_auto] gap-2 rounded-2xl border border-gray-200 bg-gray-50 p-3 text-xs font-black text-gray-500">
                  <div className="rounded-xl bg-white px-3 py-2">搜索老师 / 内容</div>
                  <div className="rounded-xl bg-white px-3 py-2">全部版本</div>
                  <div className="rounded-xl bg-white px-3 py-2">全部状态</div>
                  <button type="button" className="h-9 rounded-xl bg-gray-950 px-4 text-white">查询</button>
                </div>
                <div className="overflow-hidden rounded-2xl border border-gray-200">
                  <div className="grid grid-cols-[0.72fr_0.78fr_0.58fr_1.55fr_0.38fr_0.48fr_0.52fr_0.92fr] border-b border-gray-100 bg-gray-50 px-4 py-3 text-xs font-black text-gray-500">
                    <div>老师姓名</div>
                    <div>手机号</div>
                    <div>版本</div>
                    <div>反馈内容</div>
                    <div className="text-right">图片</div>
                    <div>时间</div>
                    <div>状态</div>
                    <div>操作</div>
                  </div>
                  {teacherSuggestionFeedbackRows.map((row) => (
                    <div key={row.id} className="grid grid-cols-[0.72fr_0.78fr_0.58fr_1.55fr_0.38fr_0.48fr_0.52fr_0.92fr] items-center border-b border-gray-100 px-4 py-3 text-sm font-medium last:border-b-0">
                      <div className="truncate pr-2 font-black text-gray-900">{row.teacher}</div>
                      <div className="truncate pr-2 text-gray-700">{row.phone}</div>
                      <div className="truncate pr-2">{row.version}</div>
                      <div className="truncate pr-3 text-gray-700">{row.content}</div>
                      <div className="pr-4 text-right font-black">{row.imageCount}</div>
                      <div className="truncate pr-2 text-gray-500">{row.time}</div>
                      <div className={cx('font-black', row.status === '待处理' ? 'text-gray-950' : 'text-gray-400')}>{row.status}</div>
                      <button
                        type="button"
                        className="h-8 justify-self-start rounded-xl border border-gray-200 px-3 text-xs font-black text-gray-700 active:bg-gray-100"
                        onClick={() => markSuggestionFeedbackStatus(row.id, row.status === '待处理' ? '已处理' : '待处理')}
                      >
                        {row.status === '待处理' ? '标记为已处理' : '标记为待处理'}
                      </button>
                    </div>
                  ))}
                </div>
  ...
  const renderSchoolAdminPrototype = () => {
    const currentSpace = teacherSpaces.find((space) => space.id === currentSpaceId) ?? teacherSpaces[0];
    const currentSpaceIsPersonal = currentSpace.type === 'personal';
    const schoolAdminLoginTabs: Array<{ key: SchoolAdminLoginTab; label: string }> = [
      { key: 'password', label: '密码登录' },
      { key: 'sms', label: '验证码登录' },
      { key: 'scan', label: '扫码登录' },
    ];
    const schoolAdminApplications = [
      { key: 'admin', label: '管理后台', meta: '开通学生：2530', enabledInPersonal: false },
      { key: 'screen', label: '课堂大屏', meta: '有效期至：2027-09-01', enabledInPersonal: true },
    ];
    const schoolAdminPromoCards = ['积分银行', 'AI素养评价', '素养指南针'];
    const schoolAdminWorkspaceTabs = currentSpaceIsPersonal ? ['首页', '课堂大屏'] : ['首页', '学校管理后台', '积分银行（一体机）', '课堂大屏'];
    const visibleSchoolAdminApplications = schoolAdminApplications.filter((item) => !currentSpaceIsPersonal || item.enabledInPersonal);
    const selectSchoolAdminSpace = (spaceId: TeacherSpaceId) => {
      setCurrentSpaceId(spaceId);
      setShowSchoolAdminSpaceMenu(false);
    };

    if (!schoolAdminLoggedIn) {
      return (
        <div className="relative flex h-full w-full max-w-[760px] items-center justify-center overflow-hidden rounded-2xl border border-gray-200 bg-[#f5f7fa] text-gray-950">
          <div className="absolute left-[14%] top-[13%] h-[180px] w-[180px] rounded-full bg-gray-200/70 blur-[58px]" aria-hidden="true" />
          <div className="absolute bottom-[12%] right-[18%] h-[160px] w-[160px] rounded-full bg-gray-100 blur-[58px]" aria-hidden="true" />
          <section className="relative z-10 w-full max-w-[440px] rounded bg-white/95 p-8 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.08)] ring-1 ring-white/70">
            <div className="mb-8 flex items-center gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded bg-[#2a68ff] text-white shadow-md shadow-blue-200">
                <Shield size={24} />
              </div>
              <div>
                <h2 className="text-xl font-black tracking-tight text-gray-900">乐途AI智慧教育平台</h2>
                <p className="mt-0.5 text-[11px] font-medium text-gray-500">学校端安全登录</p>
              </div>
            </div>

            <div className="mb-6 flex gap-3">
              {schoolAdminLoginTabs.map((item) => (
                <button
                  key={item.key}
                  type="button"
                  onClick={() => setSchoolAdminLoginTab(item.key)}
                  className={cx(
                    'flex-1 rounded border py-2.5 text-sm font-bold transition-all',
                    schoolAdminLoginTab === item.key
                      ? 'border-[#2a68ff] bg-[#2a68ff] text-white shadow-md shadow-blue-200'
            ...
            {schoolAdminLoginTab === 'scan' ? (
              <div className="space-y-4">
                <div className="mx-auto grid h-40 w-40 grid-cols-5 gap-1 rounded border border-gray-200 bg-white p-4">
                  {Array.from({ length: 25 }).map((_, index) => (
                    <span key={index} className={cx('rounded-[4px]', [0, 1, 3, 5, 6, 7, 9, 11, 13, 14, 17, 18, 20, 21, 23, 24].includes(index) ? 'bg-gray-900' : 'bg-gray-100')} />
                  ))}
                </div>
                <div className="h-4" />
                <button type="button" onClick={() => setSchoolAdminLoggedIn(true)} className="flex h-10 w-full items-center justify-center rounded bg-[#4f46e5] text-sm font-bold text-white shadow-md shadow-indigo-500/20 active:scale-[0.98]">已扫码，立即登录</button>
              </div>
            ) : (
              <div className="space-y-4">
                <div>
                  <label className="mb-1.5 flex text-[11px] font-bold text-gray-700">
                    <span className="mr-0.5 text-red-500">*</span>
                    手机号
                  </label>
                  <div className="flex h-[42px] items-center gap-3 rounded border border-gray-200 bg-white px-3 text-sm font-medium text-gray-400">
                    <UserPlus size={16} />
                    <span>请输入手机号（演示环境任意输入）</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="mb-1.5 flex text-[11px] font-bold text-gray-700">
                    <span className="mr-0.5 text-red-500">*</span>
                    {schoolAdminLoginTab === 'password' ? '密码' : '验证码'}
                  </label>
                  <div className="flex h-[42px] items-center gap-3 rounded border border-gray-200 bg-white px-3 text-sm font-medium text-gray-400">
                    <Shield size={16} />
                    <span>{schoolAdminLoginTab === 'password' ? '请输入密码（演示环境任意输入）' : '请输入验证码（演示验证码：123456）'}</span>
                    {schoolAdminLoginTab === 'sms' && <span className="ml-auto rounded bg-blue-50 px-2 py-1 text-[11px] font-bold text-[#2a68ff]">获取验证码</span>}
                  </div>
                </div>

                <div className="h-4" />
                <button type="button" onClick={() => setSchoolAdminLoggedIn(true)} className="flex h-10 w-full items-center justify-center rounded bg-[#4f46e5] text-sm font-bold text-white shadow-md shadow-indigo-500/20 active:scale-[0.98]">立即登录</button>
                <div className="flex justify-end pt-1">
                  <button type="button" className="text-[11px] font-bold text-[#2a68ff]">忘记密码？</button>
                </div>
              </div>
            )}
          </section>
    ...
    return (
      <div className="h-full w-full max-w-[760px] overflow-hidden rounded-2xl border border-gray-200 bg-white text-gray-950">
        <div className="relative flex h-14 items-center border-b border-gray-200 px-5">
          ...
          <div className="mx-5 flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
            {schoolAdminWorkspaceTabs.map((tab, index) => (
              <button
                key={tab}
                type="button"
                className={cx(
                  'h-9 shrink-0 rounded-xl border px-3 text-xs font-black',
                  index === 0 ? 'border-gray-900 bg-gray-950 text-white' : 'border-gray-200 bg-gray-50 text-gray-500'
                )}
              >
                {tab}
              </button>
          ...
          <div className="flex shrink-0 items-center gap-3">
            <div className="relative">
              <button
                ...
                className="flex h-10 min-w-[172px] items-center gap-3 rounded-2xl border border-gray-200 bg-white px-3 text-left active:bg-gray-50"
                aria-label="切换空间"
                aria-expanded={showSchoolAdminSpaceMenu}
              >
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gray-100 text-xs font-black">{getTeacherSpaceInitial(currentSpace)}</span>
                <span className="min-w-0 flex-1">
                  <span className="block text-xs font-black leading-4">班级来源</span>
                  <span className="block truncate text-[11px] font-black leading-4 text-gray-500">{currentSpace.title}</span>
                </span>
                  ...
                  {teacherSpaces.map((space) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={cx('shrink-0 rounded-lg px-2 py-1 text-[10px] font-black', active ? 'bg-white text-gray-950' : 'bg-gray-100 text-gray-500')}>{getTeacherSpaceTag(space)}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            <button type="button" onClick={() => setSchoolAdminLoggedIn(false)} className="h-10 rounded-2xl px-3 text-sm font-black text-gray-500">退出</button>
          </div>
        </div>

        <div className="h-[calc(100%-56px)] overflow-y-auto bg-gray-50 p-5">
          <section className="rounded-xl border border-gray-200 bg-white p-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                <div className="text-xl font-black">郭老师，您好！</div>
                <div className="mt-2 flex items-center gap-2 text-xs font-black text-gray-500">
                  <span className="rounded-full bg-gray-100 px-3 py-1">{currentSpace.title}</span>
                  <span className="rounded-full bg-gray-100 px-3 py-1">{currentSpaceIsPersonal ? '个人版' : '学校版'}</span>
                </div>
              </div>
              <div className="hidden grid-cols-3 gap-2 text-xs font-black text-gray-500 sm:grid">
                {schoolAdminPromoCards.map((item) => (
          ...
          <div className="mt-4 grid grid-cols-[minmax(0,1fr)_260px] gap-4">
            <div className="space-y-4">
              <section className="rounded-xl border border-gray-200 bg-white p-4">
                <div className="text-base font-black">我的应用</div>
                <div className="mt-3 grid grid-cols-2 gap-3">
                  {visibleSchoolAdminApplications.map((item) => (
                    <button key={item.key} type="button" className="min-h-[132px] rounded-xl border border-gray-200 bg-gray-50 p-4 text-left active:bg-gray-100">
                      <span className="block text-lg font-black">{item.label}</span>
                      <span className="mt-3 block text-xs font-black text-gray-500">{item.meta}</span>
                      <span className="mt-6 block h-2 w-20 rounded-full bg-gray-200" />
                    </button>
                  ))}
                </div>
              </section>
              <section className="rounded-xl border border-gray-200 bg-white p-4">
                <div className="text-base font-black">更多应用</div>
                <div className="mt-3 grid grid-cols-2 gap-3">
                  {['绘本工坊'].map((item) => (
                    <div key={item} className="rounded-xl border border-gray-200 bg-gray-50 p-3">
            ...
            <aside className="space-y-3">
              <section className="rounded-xl border border-gray-200 bg-white p-3">
                <div className="text-sm font-black">服务与支持</div>
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-black text-gray-500">
                  <div className="rounded-xl bg-gray-50 px-3 py-2">曹老师</div>
                  <div className="rounded-xl bg-gray-50 px-3 py-2">贾老师</div>
                </div>
              </section>
              {schoolAdminPromoCards.map((item) => (
                <section key={item} className="min-h-[112px] rounded-xl border border-gray-200 bg-white p-3 text-center">
                  <div className="mx-auto h-12 w-12 rounded-xl bg-gray-100" />
                  <div className="mt-3 text-sm font-black">{item}</div>
                  <div className="mx-auto mt-2 h-2 w-16 rounded-full bg-gray-100" />
                </section>
              ))}
            </aside>
          </div>
    ...
    return (
      <PhoneMockup showDeviceFrame={false} safeAreaTop={false}>
        <div className="relative h-full w-full overflow-hidden bg-white">
          ...
          {renderPhonePage()}
          {classActionToast && (
            <div role="status" aria-live="polite" className="absolute inset-x-12 top-20 z-50 rounded-2xl bg-gray-950 px-4 py-3 text-center text-xs font-black text-white shadow-[0_16px_36px_rgba(15,23,42,0.2)]">
              {classActionToast}
            </div>
          )}
  ...
  const renderPrdPanel = () => {
    ...
    const isClassDetailPrd = page === 'classDetail' || page === 'classDetailMember' || page === 'classDetailSchoolHead' || page === 'classDetailDeputy';
  ...
  const renderPhoneLoginSheet = () => {
    ...
    const dismissPhoneLoginKeyboard = () => {
    ...
    return (
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="手机号登录注册">
        <div aria-hidden="true" className="absolute inset-0" onPointerDown={dismissPhoneLoginKeyboard} />
        <section className={cx(bottomSheetPanelClass, 'relative rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          <div className="mx-auto mb-4 h-1.5 w-12 rounded-full bg-gray-300" aria-hidden="true" />
          <button
            type="button"
            aria-label="关闭手机号登录弹窗"
            onClick={() => setShowPhoneLoginSheet(false)}
            className="absolute right-3 top-2 flex h-11 w-11 items-center justify-center rounded-full text-gray-500 active:bg-gray-100"
          >
            <X size={18} aria-hidden="true" />
          </button>

          <div className="mt-4 grid grid-cols-2 rounded-2xl bg-gray-50 p-1 text-sm font-black">
            <button
              type="button"
  ...
  const renderSchoolNameSheet = () => {
    if (!showSchoolNameSheet) return null;
    const close = () => setShowSchoolNameSheet(false);
    const canSubmit = Boolean(draftTeacherSchoolName.trim());

    return (
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="修改学校">
        <button type="button" aria-label="关闭学校编辑弹窗" className="absolute inset-0 cursor-default" onClick={close} />
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          <div className="mx-auto mb-4 h-1.5 w-12 rounded-full bg-gray-300" aria-hidden="true" />
          <h3 className="text-base font-black">修改学校</h3>
          <label className="mt-4 block text-xs font-black text-gray-600">
            学校
            <input
              className="mt-2 h-12 w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 text-base font-normal text-gray-950 outline-none focus:border-gray-900"
              value={draftTeacherSchoolName}
              onChange={(event) => setDraftTeacherSchoolName(event.target.value)}
              placeholder="请输入学校名称"
              autoFocus
            />
          </label>
  ...
  const renderSpaceSelectSheet = () => {
    if (!showSpaceSelectSheet || !userHasMultipleClassSources) return null;
    const close = () => {
      setShowSpaceSelectSheet(false);
    };
    const selectSpace = (spaceId: TeacherSpaceId) => selectTeacherSpace(spaceId, page === 'mineSchool' ? 'mineSchool' : undefined);

    return (
      <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="切换班级来源">
        <button type="button" aria-label="关闭班级来源弹窗" className="absolute inset-0 cursor-default" onClick={close} />
        <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
          <div className="mx-auto mb-4 h-1.5 w-12 rounded-full bg-gray-300" aria-hidden="true" />
          <h3 className="text-base font-black">切换班级来源</h3>
          <div className="mt-4 space-y-2">
            {teacherSpaces.map((space) => {
              const active = currentSpaceId === space.id;
              return (
  ...
  const renderPhonePage = () => {
    ...
    if (page === 'profile' || page === 'profileInvite') {
      const canEnterExperience = Boolean(teacherName.trim() && teacherSchoolName.trim());
      const isInviteProfile = page === 'profileInvite';
      const inviteProfileSummary = `${inviteClass.inviter}邀请你加入「${inviteClass.name}」`;
      const inviteSubjectOptions = ['语文', '数学', '英语', '科学', '体育', '艺术'];
      const toggleInviteSubject = (subject: string) => {
        setInviteTeachingSubjects((subjects) => (
          subjects.includes(subject) ? subjects.filter((item) => item !== subject) : [...subjects, subject]
        ));
      };
      return (
        <>
          ...
          <div className="flex h-[calc(100%-64px)] flex-col p-5">
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pb-4">
              {isInviteProfile && (
                <section className="rounded-2xl bg-gray-50 p-4" aria-label="班级邀请信息">
                  <div className="flex items-start gap-3">
                    <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white text-gray-700">
                      <Building2 size={18} aria-hidden="true" />
                    </span>
                    <div className="min-w-0 flex-1">
                      <div className="text-sm font-black leading-5">{inviteClass.name}</div>
                      <div className="mt-1 text-xs font-medium leading-5 text-gray-600">
              ...
              <label className="block rounded-2xl bg-gray-50 p-3 text-xs font-black">
                ...
                <input
                  ...
                  placeholder="请输入学校名称"
                />
                {!isInviteProfile && <span className="mt-2 block text-xs font-normal text-gray-500">填写学校后，可更快找到同校老师创建的班级</span>}
              </label>
              {isInviteProfile && (
                <fieldset className="rounded-2xl bg-gray-50 p-3">
                  <legend className="px-1 text-xs font-black">任教学科（选填）</legend>
                  <div className="mt-2 grid grid-cols-3 gap-2">
                    {inviteSubjectOptions.map((subject) => {
                      const selected = inviteTeachingSubjects.includes(subject);
                      return (
                        <button
                          key={subject}
                          type="button"
                          onClick={() => toggleInviteSubject(subject)}
                          className={cx('min-h-11 rounded-xl border text-xs font-black', selected ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-200 bg-white text-gray-700 active:bg-gray-100')}
    ...
    if (page === 'classCreate') {
      return (
        <>
          <ScreenHeader title="创建班级" onBack={returnToTeacherHome} backLabel="返回首页" />
          <div className="flex h-[calc(100%-64px)] flex-col p-5">
    ...
    if (page === 'classJoin') {
      ...
      return (
        <>
          <ScreenHeader title="加入班级" onBack={returnToTeacherHome} backLabel="返回首页" />
          <div className="flex h-[calc(100%-64px)] flex-col p-5">
            <div className="flex-1 space-y-3">
              <label className="block rounded-2xl bg-gray-50 p-3 text-xs font-black">
                班级号
    ...
    if (page === 'studentAdd') {
      ...
      return (
        <>
          <ScreenHeader title="添加学生" onBack={returnToTeacherHome} backLabel="返回首页" />
          <div className="flex h-[calc(100%-64px)] flex-col p-5">
            <div className="flex-1 space-y-3 overflow-y-auto pb-4">
              ...
              <div className="px-1 text-xs font-normal text-gray-500">学号必填，填写后可支持学号进行评价，例如“1号和3号同学打架”</div>
              <button
                type="button"
            ...
            <div className="flex items-center justify-between gap-2 text-xs">
              ...
              {userHasMultipleClassSources && (
                <button
                  ...
                  className="inline-flex min-w-0 max-w-[190px] items-center gap-1.5 rounded-xl bg-gray-50 px-3 py-2 font-black text-gray-700 active:bg-gray-100"
                  aria-label="切换班级来源"
                  aria-expanded={showSpaceSelectSheet}
                >
                  <span className="min-w-0 truncate">{getTeacherSpaceDisplayName(currentSpace)}</span>
    ...
    if (isClassActionPreviewPage(page)) {
      return <div className="h-full w-full bg-gray-50" aria-hidden="true" />;
    }
    ...
    if (isPersonalClassListPage(page) || isSchoolClassListPage(page)) {
      ...
      const classCards = [
        ...
        { name: '2023级3班', code: '41862753', stage: '小学' as const, entryYearValue: 2023, tags: ['英语'], count: 36, creatorName: '李老师', isCreator: false, role: 'teacher' as const },
      ];
      ...
      return (
        <>
          <div className="space-y-3 p-5 pb-24">
            {(userHasMultipleClassSources || (!isSchoolList && isPersonalOwnedSource)) && (
              <section className="flex items-center justify-between gap-3">
                {userHasMultipleClassSources ? (
                  <button
                    type="button"
                    onClick={() => setShowSpaceSelectSheet(true)}
                    className="inline-flex h-10 min-w-0 max-w-[220px] items-center gap-2 rounded-full bg-white px-3 text-left text-sm font-black text-gray-800 shadow-[0_8px_24px_rgba(15,23,42,0.06)] ring-1 ring-gray-100 active:bg-gray-100"
                    aria-label="切换班级来源"
                    aria-expanded={showSpaceSelectSheet}
                  >
                    <span className="shrink-0 text-xs font-medium text-gray-500">班级来源</span>
                    <span className="min-w-0 truncate">{getTeacherSpaceDisplayName(currentSpace)}</span>
                    <ChevronDown size={14} className="shrink-0 text-gray-400" />
                  </button>
                ) : (
                ...
                {!isSchoolList && isPersonalOwnedSource && (
                  <button
                    type="button"
                    onClick={() => setShowPersonalClassActionSheet(true)}
                    className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-gray-900 shadow-[0_8px_24px_rgba(15,23,42,0.08)] ring-1 ring-gray-100 active:bg-gray-100"
                    aria-label="打开班级操作"
                  >
                    <Plus size={20} />
                  </button>
          ...
          {showPersonalClassActionSheet && (
            <div className={cx(bottomSheetBackdropClass, 'z-[115] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="班级操作">
              <button type="button" aria-label="关闭班级操作" className="absolute inset-0 cursor-default" onClick={() => setShowPersonalClassActionSheet(false)} />
              <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
                <div className="mb-4 flex items-center justify-between gap-3">
                  <h3 className="text-base font-black">班级操作</h3>
                  <button type="button" onClick={() => setShowPersonalClassActionSheet(false)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gray-50 text-lg font-black active:bg-gray-100">×</button>
                </div>
                <div className="space-y-2">
                  <button
                    type="button"
                    onClick={() => {
                      setShowPersonalClassActionSheet(false);
                      navigate('classCreate');
                    }}
                    className="flex h-12 w-full items-center justify-between rounded-2xl bg-gray-50 px-4 text-sm font-black active:bg-gray-100"
                  >
                    <span className="flex items-center gap-2"><Plus size={17} />创建班级</span>
                    <ChevronRight size={16} />
                  </button>
                  <button
                    type="button"
                    ...
                    className="flex h-12 w-full items-center justify-between rounded-2xl bg-gray-50 px-4 text-sm font-black active:bg-gray-100"
                  >
                    <span className="flex items-center gap-2"><LogIn size={17} />加入班级</span>
                    <ChevronRight size={16} />
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      setShowPersonalClassActionSheet(false);
                      setShowPersonalClassDisplaySheet(true);
                    }}
                    className="flex h-12 w-full items-center justify-between rounded-2xl bg-gray-50 px-4 text-sm font-black active:bg-gray-100"
                  >
                    <span className="flex items-center gap-2"><SlidersHorizontal size={17} />显示设置</span>
                    <ChevronRight size={16} />
                  </button>
                </div>
              </section>
          ...
          {showPersonalClassDisplaySheet && (
            <div className={cx(bottomSheetBackdropClass, 'z-[120] bg-gray-900/45')} role="dialog" aria-modal="true" aria-label="显示班级">
              <button type="button" aria-label="关闭显示班级设置" className="absolute inset-0 cursor-default" onClick={() => setShowPersonalClassDisplaySheet(false)} />
              <section className={cx(bottomSheetPanelClass, 'rounded-t-[28px] border border-gray-200 bg-white px-5 pb-7 pt-5 shadow-[0_-16px_40px_rgba(0,0,0,0.10)]')}>
                <div className="mb-4 flex items-center justify-between gap-3">
                  <h3 className="text-base font-black">显示班级</h3>
                  <button type="button" onClick={() => setShowPersonalClassDisplaySheet(false)} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gray-50 text-lg font-black active:bg-gray-100">×</button>
                </div>
                <div className="space-y-2">
                  {personalClassCards.map((item) => {
                    const checked = personalVisibleClassCodes.includes(item.code);
                    return (
                      <button
                        key={item.code}
                        type="button"
                        onClick={() => setPersonalVisibleClassCodes((codes) => (
                          codes.includes(item.code)
                            ? codes.filter((code) => code !== item.code)
                            : [...codes, item.code]
                        ))}
                        className="flex min-h-[58px] w-full items-center justify-between gap-3 rounded-2xl bg-gray-50 px-4 text-left active:bg-gray-100"
                ...
                <button type="button" onClick={() => setShowPersonalClassDisplaySheet(false)} className="mt-4 h-12 w-full rounded-2xl bg-gray-900 text-sm font-black text-white active:bg-gray-700">完成</button>
              </section>
            </div>
          )}
          <BottomNav />
    ...
    if (page === 'classDetail' || page === 'classDetailMember' || page === 'classDetailSchoolHead' || page === 'classDetailDeputy') {
      ...
      const classDangerLabel = page === 'classDetail' ? '解散班级' : '退出班级';
      const teacherListPage: PageKey = classRole === 'headTeacher' ? 'teacherList' : classRole === 'deputyHeadTeacher' ? 'teacherListDeputy' : 'teacherListMember';
      const parentBindingPage: PageKey = permissions.canManageParentBindings ? 'parentBindingList' : 'parentBindingListMember';
            ...
            <div className="flex-1 space-y-3 overflow-y-auto pb-4">
                ...
                <div className="space-y-3 pr-12">
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3 text-xs font-medium text-gray-600">
                      <button
                        type="button"
                        onClick={() => void copyClassCode(activeClassProfile.code)}
                        className="flex min-h-11 min-w-0 items-center gap-1.5 text-left tabular-nums active:text-gray-950"
                        aria-label="复制班级号"
                      >
              ...
              <section className="rounded-2xl bg-gray-50 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => navigate(teacherListPage)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-white active:bg-gray-100" aria-label="查看更多老师">
                    <ChevronRight size={17} />
                  </button>
                </div>
                <div className="mt-3 flex gap-2 overflow-hidden">
                  {classTeachers.slice(0, 5).map((teacher) => (
                    <div key={teacher.id} className="w-[62px] shrink-0 text-center">
              ...
              <section className="rounded-2xl bg-gray-50 p-4">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => navigate(parentBindingPage)} className="flex h-10 w-10 items-center justify-center rounded-xl bg-white active:bg-gray-100" aria-label="查看更多家长绑定情况">
                    <ChevronRight size={17} />
                  </button>
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2">
    ...
    if (page === 'parentBindingList' || page === 'parentBindingListMember') {
      ...
      const isParentBindingManager = page === 'parentBindingList';
    ...
    if (page === 'minePersonal' || page === 'mineSchool') {
      ...
      const currentSpaceTitle = getTeacherSpaceDisplayName(currentSpace);
      const managementToolItems: Array<{ key: string; label: string; icon: React.ElementType; onClick: () => void }> = [
        ...(hasMultipleVersions && isCurrentSchoolVersion ? [{
          ...
          onClick: () => showClassActionToast('学校报表演示中'),
        }] : []),
        {
          key: 'finalReport',
          label: '期末报告',
          ...
          <div className="space-y-3 p-5 pb-24">
            <div className="flex items-start gap-3 px-1 pb-1 pt-1">
              ...
              <div className="min-w-0 flex-1">
                  ...
                  {!userHasMultipleClassSources && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      <span className="rounded-lg bg-white px-2 py-1 text-xs font-black">{currentSpaceTitle}</span>
                    </div>
                  )}
                </div>
        ...
        <>
          ...
          <div className="teacher-basic-info-lowfi space-y-3 bg-gray-50 p-4 pb-24">
            <section className="overflow-hidden rounded-3xl bg-white">
              ...
              {isPersonalBasicInfo ? (
                <button type="button" onClick={openSchoolNameSheet} className="flex min-h-[52px] w-full items-center justify-between gap-3 border-t border-gray-100 px-4 text-left active:bg-gray-50" aria-label="修改学校">
    ...
    if (page === 'suggestionFeedback') {
      const canSubmitSuggestion = Boolean(suggestionFeedbackText.trim());
      const canAddSuggestionImage = suggestionFeedbackImages.length < 5;

      return (
        <>
          <ScreenHeader title="建议反馈" />
          <div className="flex h-[calc(100%-64px)] flex-col p-5">
            <div className="flex-1 space-y-4 overflow-y-auto pb-4">
              <label className="block">
                <span className="text-sm font-black text-gray-900">反馈内容</span>
                <textarea
                  value={suggestionFeedbackText}
                  onChange={(event) => setSuggestionFeedbackText(event.target.value)}
                  className="mt-2 min-h-[156px] w-full resize-none rounded-2xl border border-gray-200 bg-gray-50 p-3 text-sm font-normal leading-6 text-gray-900"
              ...
              <section>
                <div className="flex items-center justify-between">
                  <div className="text-sm font-black text-gray-900">上传图片</div>
                  <div className="text-xs font-black text-gray-500">{suggestionFeedbackImages.length}/5</div>
                </div>
                <div className="mt-2 grid grid-cols-3 gap-2">
                  {suggestionFeedbackImages.map((image, index) => (
                    <div key={`${image}-${index}`} className="relative flex aspect-square items-center justify-center rounded-2xl bg-gray-50 text-xs font-black text-gray-500">
                      {image}
                      <button
                        type="button"
                        onClick={() => removeSuggestionFeedbackImage(index)}
                        className="absolute -right-1 -top-1 flex h-7 w-7 items-center justify-center rounded-full bg-white text-gray-700 shadow-[0_4px_12px_rgba(15,23,42,0.12)]"
                        aria-label={`删除第${index + 1}张反馈图片`}
                      >
                        ×
                      </button>
                    </div>
                  ))}
                  {canAddSuggestionImage && (
                    <button
                      type="button"
                      onClick={addSuggestionFeedbackImage}
                      className="flex aspect-square flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-gray-300 bg-white text-xs font-black text-gray-600 active:bg-gray-50"
                      aria-label="上传反馈图片"
                    >
                      <ImagePlus size={22} />
                      上传图片
            ...
            <button
              ...
              className={cx('h-12 w-full rounded-2xl border border-gray-200 text-sm font-black', canSubmitSuggestion ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-400')}
            >
              提交
            </button>
          </div>
