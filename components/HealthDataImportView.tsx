...
import { Alert, Button, Select, Table, Tag } from '@arco-design/web-react';
...
const HealthDataImportView: React.FC = () => {
    ...
    <div className="mb-4 flex items-center gap-3">
      ...
      <div>
        <div className="mb-1 flex items-center gap-2 text-[13px] text-[#86909C]">
          <span>数据中心</span><span>/</span><span className="text-[#4E5969]">体检数据</span>
        </div>
  ...
  if (pageMode === 'upload') {
    return (
      <div className="w-full px-6 py-5 text-sm text-[#4E5969]">
        <PageTitle title="导入体检数据" onBack={resetImport} />
        <section className="rounded border border-[#E5E6EB] bg-white p-6">
          ...
          <div className="mx-auto max-w-[720px]">
            ...
            >
              ...
              <span className="text-[15px] font-semibold text-[#1D2129]">选择体检数据文件</span>
              <span className="mt-2 text-[13px] text-[#86909C]">支持平台固定Excel模板</span>
            </button>
            {fileName && (
      ...
      <div className="w-full px-6 py-5 text-sm text-[#4E5969]">
        ...
        <section className="rounded border border-[#E5E6EB] bg-white p-6">
          <div className="flex items-start gap-4 border-b border-[#E5E6EB] pb-6">
            ...
            <div>
              <h2 className="m-0 text-[18px] font-semibold text-[#1D2129]">体检数据已导入</h2>
          ...
          <div className="mt-6 grid max-w-[760px] grid-cols-2 gap-x-12 gap-y-4">
            {[
              ['写入学生体检记录', `${activeBatch.writtenRecords}条`],
          ...
          <div className="mt-6 border-t border-[#E5E6EB] pt-4"><Button type="primary" onClick={() => setPageMode('list')}>返回体检数据</Button></div>
        </section>
      </div>
    );
  }
    ...
    <div className="w-full px-6 py-5 text-sm text-[#4E5969]">
      ...
      <section className="rounded border border-[#E5E6EB] bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          ...
          <Button type="primary" icon={<Upload size={16} />} onClick={() => setPageMode('upload')}>导入体检数据</Button>
        </div>
