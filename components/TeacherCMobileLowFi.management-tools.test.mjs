...
requireText("modules: ['教师信息', '头像', '姓名', '扫一扫', '设置', '了解学校版', '管理工具', '期末报告', '更多工具', '科目管理', '部门管理', '校园币发放管理'", '14A 应保留独立管理工具，并在更多工具中新增校园币发放管理。');
requireText("modules: ['教师信息', '头像', '姓名', '扫一扫', '设置', '班级来源入口', '管理工具', '学校报表', '期末报告', '更多工具', '科目管理', '部门管理', '校园币发放管理'", '14B 应保留独立管理工具，学校版展示学校报表，并在更多工具中保留校园币发放管理。');
...
requireText("label: '学校报表'", '14B 当前切换到学校版时，管理工具应展示学校报表。');
requireText("hasMultipleVersions && isCurrentSchoolVersion", '学校数据报表只应在 14B 且当前切换到学校版时展示。');
requireText("showClassActionToast('学校报表演示中')", '学校报表入口应提供演示反馈。');
requireText("label: '校园币发放', icon: Coins, onClick: () => navigate('coinIssuanceManagement')", '校园币发放入口应进入 21 货币发放页。');
...
requireText('开启校园币发放', '货币发放页应展示顶部校园币开关配置项。');
...
requireText('h-11 w-[172px] max-w-full justify-self-end rounded-xl border border-gray-200 bg-gray-50', '发放周期选择器应与门槛配置段宽度、高度和圆角统一。');
requireText('发放门槛', '发放条件应展示发放门槛。');
requireText("coinIssuePeriod === 'weekly' ? '本周至少评价' : '本月至少评价'", '发放门槛文案应随发放周期切换。');
requireText('张老师评价：全班50名同学积极参加锻炼，算作1次评价', '发放门槛应说明一次全班评价的计数规则。');
requireText('whitespace-nowrap pl-1 text-xs font-medium leading-5 text-gray-500', '发放门槛提示应压缩为单行文案。');
requireText('h-11 w-11 shrink-0 rounded-xl border border-gray-200 bg-white', '次数输入框应保持当前高度并收窄为等高方框。');
requireText('value={coinBudgetDraft}', '发放数量应使用数字输入状态。');
requireText('发放数量', '发放数量区域应使用新标题。');
requireText('按人', '发放数量应提供按人页签。');
requireText('按班', '发放数量应提供按班页签。');
requireText('role="group" aria-label="预算设置"', '发放数量页签容器应使用设置项分组语义。');
requireText('grid min-h-12 grid-cols-[minmax(72px,1fr)_minmax(0,1.8fr)] items-center gap-3 px-1', '预算设置应与发放条件保持左标签右配置项的同行布局。');
requireText('flex w-36 items-center gap-2', '预算输入框和币单位整体宽度应与预算设置 Tab 的 144px 宽度一致。');
requireText('<span className="shrink-0 text-sm font-black text-gray-700">币</span>', '预算输入框应统一展示币单位。');
requireText("const nextBudget = option.value === 'per_student' ? 10 : 500", '切换预算口径时应分别采用每人10币或每班500币默认值。');
requireText('一个班50人，则班级预算为', '预算到人应展示班级预算计算方式。');
requireText("const formatCoinAmount = (amount: number) =>", '预算试算金额应统一使用金额格式化。');
requireText('col-span-2 text-pretty text-xs font-medium leading-5 text-gray-500', '试算文案应与门槛提示保持同一左侧基线。');
...
requireText('<div className="font-medium">每个学生无论评价如何，都可以获得的成长奖励</div>', '阳光保底说明第一行应只展示成长奖励解释文案。');
requireText('<div className="font-black text-gray-900">全班平分：{formatCoinAmount(sunshinePoolAmount)}币</div>', '阳光保底金额应换行展示，并使用保底资金池金额。');
requireText('<div className="font-medium">评分为正的学生，按比例分配的排名奖励</div>', '积分排行说明第一行应只展示排名奖励解释文案。');
...
requireText("showClassActionToast('校园币发放配置已保存')", '保存按钮应给出保存反馈。');
...
requirePrd('更多工具卡片 → 由原“学校基础信息”改名而来，14A 展示科目管理、部门管理、校园币发放管理', 'PRD 应说明 14A 更多工具内容。');
requirePrd('管理工具卡片 → 14B 当前切到学校版时展示学校数据报表、生成期末报告；切到个人版时仅展示生成期末报告', 'PRD 应说明 14B 管理工具随当前版本切换。');
requirePrd('更多工具卡片 → 由原“学校基础信息”改名而来，14B 展示科目管理、部门管理、校园币发放管理', 'PRD 应说明 14B 更多工具内容。');
requirePrd('校园币发放管理 → 进入 21 货币发放，默认关闭；开启后展示发放周期、发放门槛、发放数量、阳光保底比例、积分排行比例和保存按钮', 'PRD 应说明校园币发放管理链路。');
