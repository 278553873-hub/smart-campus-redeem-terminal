...
V1 取消列表和批量排班，页面直接进入月历。“值周安排”归入“我的 → 更多工具”，并按使用频率排在货币发放、问卷采集之后。教师选择抽屉标题只显示日期范围，“暂不安排老师”固定在标题栏；正文直接从姓名搜索开始，点击后与选择教师一样立即保存并关闭。
...
| “当前安排 + 清空”操作不直观 | 抽屉标题栏固定展示“暂不安排老师”中性按钮 |
...
4. 设置或暂不安排教师：健康。“暂不安排老师”始终位于日期标题右侧；点击该选项或选择新教师后都立即保存、关闭抽屉并显示成功提示。

## Fidelity and accessibility

| 检查项 | 结果 |
| --- | --- |
| Layout | 两列 76 像素周块保持紧凑网格；抽屉标题栏承载日期、暂不安排与关闭，正文只保留搜索和教师结果 |
| Typography | 日期、周次和进度使用等宽数字；姓名与状态层级低于日期，姓名不使用简称 |
| Tokens | 周块高度、已安排与未安排表面、选中阴影、弱化透明度、本周按钮、暂不安排选项与教师行高均由 `--tm-duty-*` 管理 |
