...
> 能力子规范：本文只维护班级评价侧的数据面板、周报和对话规则。统一入口、能力组合、页面层级和产品命名以 `docs/PRD-班主任助理.md` 为准；“班主任助理 V2”不再作为用户可见产品或独立入口。
...
本文档是统一班主任助理中的班级评价能力子规范。以下旧方案仅保留为历史记录，不再作为实现依据：

- `docs/PRD-班主任助理V2-班级评价自由对话.md`
- `docs/班主任助理V2_班级评价自由对话_系统提示词.md`

旧方案中的责任划分、整改状态、负责人和申诉处理仍不属于当前能力。当前版本支持受控自由对话，但只允许查询当前班级评价数据，不恢复旧方案中超出真实数据范围的能力。
...
班级周评分析是统一班主任助理的一项数据能力。它基于班级每周得分、排名、一级指标得分和逐笔扣分记录，提供班级数据面板、快捷问题和连续对话；底层仍保留完整周报的生成与历史能力。
...
4. 主要扣分问题。
5. 下周关注重点。
6. 相关数据记录仅用于模型校验和审计，不在回答中提供“查看依据”操作。

原三个固定问题收敛为上述三个报告章节，不再作为用户入口，不分别调用模型，不分别保存历史。

#### 7.1.6 按需生成与历史报告

- 首次进入统一班主任助理只展示当前已开通能力，不自动生成任何报告。
...
- 首页在输入控件上方横向展示 3 个班级评价快捷问题：`班级评价主要扣在哪？`、`班级评价较上周哪项变化最大？`、`根据班级评价，下周优先关注什么？`。问题本身明确数据来源，不另加分类标题；问题时间范围继承上方“本周数据”上下文，不重复堆叠“本周”文案。不使用纵向大卡片占用首屏，也不与中部学生评价能力入口混排。
- 发送问题后在统一页面内追加对话，不跳转到独立模式或独立对话页面。
- 对话默认使用当前班级和当前周，页面明确展示数据截止时间。
- Agent 回答使用普通分段文字，按“直接结论 → 必要数据 → 综合判断 → 行动建议”组织；不显示头像、名称、图标标题、指标宫格、彩色文字或查看依据。
...
统一班主任助理属于 Agent，因为系统既能按固定报告任务调度工具，也能根据班主任的自然语言问题选择最小工具集合、组织上下文、调用模型并校验结果。模型不能直接访问数据库，工具选择和权限边界由编排层控制。
...
- 记录编号用于服务端校验、审计和后续工具调用，不在当前回答界面提供查看依据入口。
...
4. 一次生成能得到包含数据概览、整体表现、扣分问题和下周重点的完整周报。
5. 报告页不显示聊天气泡、“继续分析”或重复问题入口。
6. 回答和完整周报均不展示查看依据入口。
7. 首页底部支持语音和文字输入，不展示拍照和添加按钮。
8. 发送问题后在统一页面内追加回答，回答后可继续追问且不重复本轮问题。
9. 对话使用当前班级和当前周；切换班级后不得继承上一班级的消息和记录编号。
10. 打开班主任助理、切换班级、打开历史和查看历史详情均不调用模型；只有主动生成周报或主动发送问题才调用模型。
