...
班主任助理 V2 是面向班主任的班级周评分析 Agent。它基于班级每周得分、排名、一级指标得分和逐笔扣分记录，生成一份包含整体表现、主要扣分问题、下周关注重点和逐笔依据的完整周报。

V2 不开放自由聊天。班主任主动点击“生成本周班级评比分析”后，系统执行固定工具计划，后端负责计算分数和统计结果，人工智能一次生成完整周报中的分析和建议。
...
```mermaid
flowchart LR
    A["班主任点击生成本周分析"] --> B["检查班级、周、快照和整份报告提示词版本"]
    B --> C{"是否已有相同报告"}
    C -->|"是"| K["直接展示已生成报告"]
    C -->|"否"| D0["权限与数据周期校验"]
    D0 --> D["执行完整周报固定工具计划"]
    ...
    H --> I["结构和证据编号校验"]
    I --> J["合并统计与人工智能内容"]
    J --> J2["保存报告、快照、提示词版本和逐笔依据"]
    J2 --> K
```

V2 仍然属于 Agent，因为系统会根据周报任务调度数据工具、组织上下文、调用模型并校验结果。当前只有一个固定周报任务，不需要模型先做意图识别或自由选择工具。
...
| 随机性 | 低随机性，建议温度 0.2 |
| 工具选择 | 由周报任务映射固定工具计划，不让模型自由选工具 |
| 统计计算 | 全部由后端完成 |
| 模型输入 | 完整周报需要的汇总和有限逐笔摘要 |
| 模型输出 | 只包含直接结论、分析和建议 |
...
7. 页面不存在自由文字、语音和拍照入口。
8. 打开 V2、切换班级、打开历史和查看历史详情均不调用模型。
9. 用户首次主动点击未生成周报时，展示四阶段 Agent 分析过程，完成后写入历史。
10. 相同四项缓存键再次点击时直接展示已生成报告，不播放生成过程，不重复调用模型。
11. 往期报告按月份分组、按周倒序展示，历史详情直接读取生成时冻结的报告和依据。
...
- 后续只有在数据范围扩大且固定周报无法覆盖高频需求时，才重新评估自由对话能力。
