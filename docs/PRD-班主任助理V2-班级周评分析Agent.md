...
旧方案中的自由对话、连续追问、责任划分、整改状态、负责人和申诉处理均不属于当前 V2。

当前 Demo 中还没有独立的真实模型提示词调用。代码只保存了三个提示词版本编号，并用前端领域逻辑模拟结构化结果：

- 固定问题和模拟分析逻辑：`mobile-app/domain/classEvaluationAssistantV2.ts`
- 回答页面：`mobile-app/views/AiHeadteacherAssistantV2View.tsx`

本文档第 7.8 至 7.11 节定义正式接入时使用的提示词、工具、输入和输出协议。

## 1. 摘要

班主任助理 V2 是面向班主任的班级周评分析 Agent。它基于班级每周得分、排名、一级指标得分和逐笔扣分记录，通过三个固定问题提供确定性数据统计、人工智能分析、人工智能建议和逐笔数据依据。

V2 不开放自由聊天。班主任点击固定问题后，系统按问题编号调用预先配置的数据工具，后端负责计算分数和统计结果，人工智能只负责解释数据、归纳问题和给出有限建议。
...
2. 固定问题直接映射固定工具计划，不让模型自由猜测需要查询什么。
3. 分数、排名、差值、占比和排序全部由后端确定性计算。
4. 人工智能只生成分析和建议，不计算分数，不修改统计结果。
5. 页面只展示一份当前报告。点击其他固定问题时替换当前报告，不累积聊天历史。
...
| 问题编号 | 页面文案 | 提示词版本 |
|---|---|---|
| `weekly_performance` | 本周班级评比表现怎么样？ | `headteacher-class-evaluation-weekly-performance-v1` |
| `deduction_patterns` | 本周扣分反映出哪些主要问题？ | `headteacher-class-evaluation-deduction-patterns-v1` |
| `next_week_focus` | 下周应该重点关注什么？ | `headteacher-class-evaluation-next-week-focus-v1` |
...
点击其他问题时替换当前报告。页面不保留上一份报告，不显示多轮聊天气泡历史。

### 7.2 Agent 工作流

```mermaid
flowchart LR
    A["班主任点击固定问题"] --> B["提交班级、周和问题编号"]
    B --> C["权限与数据周期校验"]
    C --> D["按问题编号执行固定工具计划"]
    D --> E["后端生成确定性统计"]
    D --> F["查询相关逐笔记录"]
...
V2 仍然属于 Agent，因为系统会根据任务调度数据工具、组织上下文、调用模型并校验结果。但当前只有三个固定任务，因此工具计划由问题编号决定，不需要模型先做意图识别或自由选择工具。
...
| 固定问题 | 必调工具 | 逐笔记录范围 |
|---|---|---|
| 本周班级评比表现怎么样？ | 周快照、一级指标汇总、扣分分析 | 扣分最高的两个一级指标，最多 10 笔 |
| 本周扣分反映出哪些主要问题？ | 周快照、扣分分析 | 当前周相关扣分记录，最多 20 笔 |
| 下周应该重点关注什么？ | 周快照、一级指标汇总、周对比、扣分分析 | 最大下降项和本周扣分最高项，最多 20 笔 |
...
| 配置项 | 要求 |
|---|---|
| Agent 类型 | 固定任务、受控工具、结构化输出 |
| 模型 | 支持严格结构化输出的模型，具体型号由技术选型决定 |
| 随机性 | 低随机性，建议温度 0.2 |
| 工具选择 | 由 `question_id` 映射固定工具计划，不让模型自由选工具 |
| 统计计算 | 全部由后端完成 |
| 模型输入 | 当前问题需要的汇总和有限逐笔摘要 |
| 模型输出 | 只包含直接结论、分析和建议 |
| 输出校验 | 严格 JSON（结构化数据）校验；记录编号必须属于工具结果 |
| 重试 | 结构错误时最多重试 1 次，不扩大数据范围 |
| 超时降级 | 展示确定性统计，并提示分析暂时未生成 |
| 审计 | 保存问题编号、提示词版本、模型版本、工具调用摘要和数据快照编号 |
...
```json
{
  ...
  "prompt_version": "headteacher-class-evaluation-weekly-performance-v1",
  "week_snapshot": {},
  "dimension_summary": [],
  "deduction_analysis": {},
  "evidence_records": []
...
```json
{
  ...
  "prompt_version": "headteacher-class-evaluation-deduction-patterns-v1",
  "week_snapshot": {},
  "deduction_analysis": {},
  "evidence_records": []
}
...
```json
{
  ...
  "prompt_version": "headteacher-class-evaluation-next-week-focus-v1",
  "week_snapshot": {},
  "dimension_summary": [],
  "week_comparison": {},
  "deduction_analysis": {},
...
| 有总分但没有五项汇总 | 只展示总分和排名，三个固定问题暂不可用 |
...
- 审计日志保存：教师编号、班级编号、问题编号、提示词版本、模型版本、工具调用、快照编号、引用记录编号和耗时。
...
4. 三个固定问题均能生成“数据概览、人工智能分析、人工智能建议、查看依据”。
5. 点击其他固定问题后只保留最新报告。
...
- 后续只有在数据范围扩大且固定问题无法覆盖高频需求时，才重新评估自由对话能力。
